...
def test_displacement_reliance_blocks_covered_classification():
    record = {"primary_classification": "COVERED", "requested_deltas": [], "flags": [],
              "relied_upon": ["cue.displacement"]}
    assert "DISPLACEMENT_PROVISIONAL" in _codes(check_classification(record, "c"))


def test_displacement_reliance_ok_as_unresolved():
    record = {"primary_classification": "UNRESOLVED", "requested_deltas": ["cue.displacement semantics"],
