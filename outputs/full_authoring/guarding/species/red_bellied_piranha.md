...
- **判断顺序（REP-ORDER-FIX-002 顺序还原）**：Guard 面＝锚存在性判定 → 锚适配（沉水树根/水草丛附着面三档） → 关系评估（三档） → 局部温度 → 合并；锚不存在格 ×0.01 软出局返回（非零）。推导来源（Tier B）：R06 FR3 抽验名单一句「树根护卵」——锚型方向级；步序与档位成员 [需正文] 校准。Normal 面＝水层软定位（pelagic 中上水层） → 结构 → 水温（排除档=极值出局） → 时段（全天） → 合并——物种属性锚方向级 [需正文]。分级命中：各步三档（最适应=全额 / 可接受=削减不清零 / 排除=出局），档位成员与阈值全 Profile 值域不冻结。与 live BA-T2/BA-T1 模板平铺读法的分歧登记 README §7。
...
不进入后续评估——×0.01 软出局：非零、仍可参与下游[REP-WORDING-ALIGN-001]），不做「先全算再减」。推导来源（Tier B）：R06 FR3 抽验名单一句「树根护卵」
（沉水树根/水草丛间卵体守护——锚型方向级）；步序与档位成员 [需正文] 校准（顺序/档位变化=
...
第 1 步 锚存在性判定（GATE_ANCHOR_EXISTENCE）：
    用锚域事实查询 @PiranhaLocalGuardAnchorEligibility
    （锚=root_spawn 树根卵体；护巢资格已在路由面判定，本步不重复结算 premise，
      只判「当前目标是否处于卵体锚的合法护巢锚域」）
    如果 当前目标处于合法锚域：
        进入第 2 步
    否则：
...
第 2 步 锚适配（EVAL_ANCHOR_SUITABILITY，分级命中）：
    ...
    （树根/水草丛附着面三档分档槽=Profile 值域——档位成员与阈值不冻结 [需正文]）
    如果 锚面 ∈ 最适应档（preferred 附着面）：
        AnchorSuitabilityFit = 全额
    否则如果 ∈ 可接受档（tolerated 附着面）：
        AnchorSuitabilityFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（排除锚面）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；排除附着面出局）

第 3 步 关系评估（EVAL_ANCHOR_RELATION，分级命中）：
    用当前目标与卵体锚点的关系（距离 / 朝向）查询 @PiranhaGuardRelationProfile
    （守卫位三档=Profile 值域不冻结）
    如果 关系 ∈ 守卫核档：
        RelationFit = 全额（守卫占位）
    否则如果 ∈ 守卫缘档：
...
```plain text
【顺序还原声明｜REP-ORDER-FIX-002】Normal 面判断链＝水层定位（pelagic 软定位——中上开阔水层） →
结构 → 水温 → 时段 → 合并。推导来源：物种属性锚方向级（pelagic——CSV 行级方向，[需正文] 校准；
无 Story 空间程序证据的因子顺序不冒充）。水温从「末位算术门」还原为链中档位判定（排除档=
EARLY_RETURN，@PiranhaNormalTempFloor 为排除档边界参考）。与 live BA-T1 Independent Factor
...
第 1 步 水层定位（软定位，分级命中）：
    用当前水层查询 @PiranhaNormalLayerProfile
    （pelagic 软定位三档=Profile 值域不冻结 [需正文：定位档成员]）
    如果 水层 ∈ 中上开阔水层档：
        LayerFit = 全额
    否则如果 ∈ 近结构沿岸档：
        LayerFit = 削减（× Profile 衰减参数——不清零）
    否则（排除档）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；排除水层档出局）

第 2 步 结构（分级命中）：
    用当前结构查询 @PiranhaNormalStructureProfile（三档=Profile 值域不冻结）
    如果 结构 ∈ 最适应档：
        StructureFit = 全额
    否则如果 ∈ 可接受档：
        StructureFit = 削减（不清零）
...
第 3 步 水温（分级命中）：
    用当前水温查询 @PiranhaNormalTemperatureProfile（三档=Profile 值域不冻结）
    如果 水温 ∈ 适温档：
        TemperatureFit = 全额
    否则如果 ∈ 边际档：
        TemperatureFit = 削减（不清零）
    否则（排除档——@PiranhaNormalTempFloor 为边界参考）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；极值温度带出局）

第 4 步 时段（分级命中）：
    用当前时段查询 @PiranhaNormalTimeProfile（全天三档=Profile 值域不冻结）
    如果 时段 ∈ 活跃档：
        TimeFit = 全额
    否则如果 ∈ 一般档：
        TimeFit = 削减（不清零）
...
与 §3.1 配置表「命中 / 未命中」两列语义对齐）：
    按三档判定 FoodEvaluation（档位成员=@PiranhaNormalFeedingProfile 值域不冻结）：
...
- 使用的自由度：V1 三原子 + R1；BA-T2 锚实例=root_spawn；R-T1 Profile 重绑定；QT-1；**顺序还原链序与档位结构（REP-ORDER-FIX-002：Guard 面锚存在性→锚适配→关系→温度 early return 链、三档分级命中；Normal 面 pelagic 定位→结构→水温→时段链；Response DECIDE 三档——推导依据 §0 判断顺序行，全部 [需正文] 校准）**。
- 放弃的自由度：(1) Frenzy Group Mode（R06 Negative Knowledge 判例——无证据不建组）；(2) Defense / Feeding arbitration（live V0）；(3) Guard 合并算子数学 OPERATOR UNDEFINED；(4) live BA-T1 Independent Factor Set 无序因子语义的服从（Normal 面顺序还原链与无序合并语义拓扑分歧——登记 README §7，裁决归机制侧）；(5) 数值与 Profile 值域不冻结（含各步三档档位成员与阈值）。
