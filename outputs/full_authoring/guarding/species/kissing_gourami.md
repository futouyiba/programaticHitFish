...
不进入后续评估——×0.01 软出局：非零、仍可参与下游[REP-WORDING-ALIGN-001]），不做「先全算再减」。推导来源（Tier C 骨架）：**零证据**——链形态取
...
第 2 步 锚适配（EVAL_ANCHOR_SUITABILITY，分级命中）：
    ...
    否则（排除锚面）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；排除锚面出局 [需正文]）
...
第 2 步 结构（分级命中）：
    ...
    否则如果 ∈ 可接受档：
        StructureFit = 削减（不清零）
    否则：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；排除结构档出局 [需正文]）
...
第 4 步 时段（分级命中）：
    ...
    否则如果 ∈ 一般档：
        TimeFit = 削减（不清零）
    否则：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；排除时段档出局 [需正文]）
