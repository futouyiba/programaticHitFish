...
- **判断顺序（REP-ORDER-FIX-002 顺序还原）**：Guard 面＝锚存在性判定 → 锚适配（砾石基质三档——**构建型**：雄鱼筑巢=选址适配先行） → 关系评估（三档） → 局部温度 → 合并；锚不存在格 ×0.01 软出局返回（非零）。推导来源（Tier B）：R07 FR3 抽验名单「石巢筑造+守护」（构筑型锚——方向级）；巢体存在事实已在路由面结算（C4），Bake 不重复结算；步序与档位成员 [需正文] 校准。Normal 面＝水层硬定位（demersal——非底层出局） → 结构 → 水温（排除档=极值出局） → 时段（早晨） → 合并——物种属性锚方向级 [需正文]。分级命中：各步三档（最适应=全额 / 可接受=削减不清零 / 排除=出局），档位成员与阈值全 Profile 值域不冻结。与 live BA-T2/BA-T1 模板平铺读法的分歧登记 README §7。
...
EARLY_RETURN（返回 0.01 × weight，非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；不进入后续评估），不做「先全算再减」。推导来源（Tier B）：R07 FR3
