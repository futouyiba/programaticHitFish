...
如果当前目标不满足 @HornyheadLocalGuardAnchorEligibility：
    返回 极低 / 0 空间权重（early return）

用当前目标与石巢锚点的关系查询 @HornyheadGuardRelationProfile
得到 RelationFit
...
用当前水层查询 @HornyheadNormalLayerProfile 得到 LayerFit
用当前结构查询 @HornyheadNormalStructureProfile 得到 StructureFit
用当前水温查询 @HornyheadNormalTemperatureProfile 得到 TemperatureFit
用当前时段查询 @HornyheadNormalTimeProfile 得到 TimeFit

如果 TemperatureFit < @HornyheadNormalTempFloor：
...
评价 @HornyheadGuardThreatProfile
得到 DefenseResponse
...
- 使用的自由度：V1 四原子（巢体存在事实）+ R1；BA-T2 锚实例=stone_nest；R-T1 Profile 重绑定；QT-1。
- 放弃的自由度：(1) **guard target specificity**（R07 Open Question——对共生产卵者的分辨不表达，Profile 无特异性形态）；(2) Defense / Feeding arbitration（live V0）；(3) Guard 合并算子数学 OPERATOR UNDEFINED。
