...
- **判断顺序（REP-ORDER-FIX-002 顺序还原）**：Guard 面＝锚存在性判定 → 锚适配（砾石基质三档——**构建型**：雄鱼筑巢=选址适配先行） → 关系评估（三档） → 局部温度 → 合成；锚不存在格 ×0.01 软出局返回（非零）。推导来源（Tier B）：R08「石巢第 3 例跨属」——与 R07 双点美鱥同形处置（石巢构筑型锚，方向级）；巢体存在事实已在路由面结算（C4），Bake 不重复结算；步序与档位成员 [需正文] 校准。Normal 面＝水层硬定位（demersal——非底层出局） → 结构 → 水温（排除档=极值出局） → 时段（早晨） → 合并——物种属性锚方向级 [需正文]。分级命中：各步三档（最适应=全额 / 可接受=削减不清零 / 排除=出局），档位成员与阈值全 Profile 值域不冻结。与 live BA-T2/BA-T1 模板平铺读法的分歧登记 README §7。
...
EARLY_RETURN（返回 0.01 × weight，非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；不进入后续评估），不做「先全算再减」。推导来源（Tier B）：R08「石巢
第 3 例跨属」——与 R07 双点美鱥同形处置（雄鱼筑砾石巢并守护：构筑型锚=选址适配先于守位）；
巢体存在事实已在路由面判定（C4），本程序不重复结算该 premise；步序与档位成员 [需正文] 校准
（顺序/档位变化=census 判同输入，结构变更需重审）。与 live BA-T2 模板平铺读法的分歧登记 README §7。
...
第 1 步 锚存在性判定（GATE_ANCHOR_EXISTENCE）：
    用锚域事实查询 @CreekChubLocalGuardAnchorEligibility
...
第 2 步 锚适配（EVAL_ANCHOR_SUITABILITY，分级命中——构建型选址语义）：
    ...
    否则（排除基质档）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；排除基质出局——不可筑巢底质无石巢分布，构建型选址先行）

第 3 步 关系评估（EVAL_ANCHOR_RELATION，分级命中）：
    用当前目标与石巢锚点的关系（距离 / 朝向）查询 @CreekChubGuardRelationProfile
    （守卫位三档=Profile 值域不冻结）
    如果 关系 ∈ 守卫核档：
        RelationFit = 全额（守卫占位）
    否则如果 ∈ 守卫缘档：
...
还原为链中档位判定（排除档=EARLY_RETURN，@CreekChubNormalTempFloor 为排除档边界参考）。
...
第 1 步 水层定位（硬定位，分级命中）：
    用当前水层查询 @CreekChubNormalLayerProfile
...
第 2 步 结构（分级命中）：
    用当前结构查询 @CreekChubNormalStructureProfile（三档=Profile 值域不冻结）
    如果 结构 ∈ 最适应档：
        StructureFit = 全额
    否则如果 ∈ 可接受档：
        StructureFit = 削减（不清零）
...
第 3 步 水温（分级命中）：
    用当前水温查询 @CreekChubNormalTemperatureProfile（三档=Profile 值域不冻结）
    如果 水温 ∈ 适温档：
        TemperatureFit = 全额
    否则如果 ∈ 边际档：
        TemperatureFit = 削减（不清零）
    否则（排除档——@CreekChubNormalTempFloor 为边界参考）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；极值温度带出局）

第 4 步 时段（分级命中）：
    用当前时段查询 @CreekChubNormalTimeProfile（早晨三档=Profile 值域不冻结）
    如果 时段 ∈ 活跃档：
        TimeFit = 全额
    否则如果 ∈ 一般档：
        TimeFit = 削减（不清零）
...
与 §3.1 配置表「命中 / 未命中」两列语义对齐）：
    按三档判定 FoodEvaluation（档位成员=@CreekChubNormalFeedingProfile 值域不冻结）：
...
- 使用的自由度：V1 四原子 + R1；BA-T2 锚实例=stone_nest（与双点美鱥同模板跨属复用——石巢系第 3 例的同形处置）；R-T1 Profile 重绑定；QT-1；**顺序还原链序与档位结构（REP-ORDER-FIX-002：Guard 面锚存在性→构建型基质适配→关系→温度 early return 链、三档分级命中——链形同双点美鱥（跨属同形处置的组成部分）；Normal 面 demersal 硬定位→结构→水温→时段链；Response DECIDE 三档——推导依据 §0 判断顺序行，全部 [需正文] 校准）**。
- 放弃的自由度：(1) Defense / Feeding arbitration（live V0）；(2) Guard 合并算子数学 OPERATOR UNDEFINED；(3) 属级差异的模板分化（跨属重复＝Profile 重绑定，不建新模板——R08 同形处置口径）；(4) live BA-T1 Independent Factor Set 无序因子语义的服从（Normal 面顺序还原链与无序合并语义拓扑分歧——登记 README §7，裁决归机制侧）；(5) 数值与 Profile 值域不冻结（含各步三档档位成员与阈值）。
