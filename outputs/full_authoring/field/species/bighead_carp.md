...
- **判断顺序（REP-ORDER-FIX-005 顺序还原，handoff 判序+census 语义层推导）**：判断链＝滤食水层定位（中上层滤食带三档）→ 场浓度判定（浮游浓度三档；水流输送调制并入 Profile 值域）→ 个体大小口径判定（鳃耙口径三档）→ 归一化（族常量终点步——SINGLE 族域无 combine 步）。推导来源＝handoff 判序指令（滤食型：先判水层位置（上层/中层）→水流/食物场密度→个体大小口径→合并）落于 census P-B2-BHC-BAKE 场评估语义（三步均为同一场评估 FieldSuitability 的顺序判定，非多因子并联）；与 census sketch「高生产力水层定位＝场评估的自然输出」判语的读法差异（定位显式化为第一判定步 vs 评估输出承载）登记 README §7（census 侧族重跑裁决点）。分级命中：每步三档（偏好=全额/可接受=削减×衰减不清零/不接受=出局 EARLY_RETURN）；early return 的对象=格子。档位成员与阈值全 Profile 值域不冻结 [需正文]。
...
| 字段 | 值 |
|---|---|
| BakeTemplate | BA-P03-FIELD-SINGLE（本批投影标签＝census SINGLE_FACTOR_NORMALIZED_WEIGHT factor_type 轴 food_field 场实例，registry v4；2 步场评估→归一化；本组 Tier A 样板文件；**§2.2 已顺序还原（REP-ORDER-FIX-005）：水层定位→场浓度→口径链+early return，分歧登记 README §7**） |
...
第 1 步 滤食水层定位（场评估的前置顺序判定，分级命中）：
    用水层/深度带事实查询 @BhcPlanktonFieldEvaluatorProfile 的水层分档槽
    （中上层滤食带——滤食型先判水层位置（上层/中层）；档位成员与阈值=Profile 值域不冻结 [需正文]）
    如果 ∈ 偏好滤食水层（preferred 槽——中上层富生产力带）：
        不折减，进入第 2 步
...
第 3 步 个体大小口径判定（口径三档——契约口径过滤维度的链位化）：
    用口径过滤前后的构成对比（食物场浓度事实 vs diet_classes=@BhcDietClasses
      与 size_window=@BhcSizeWindow 过滤后的在场可食构成）查询 @BhcPlanktonFieldEvaluatorProfile
      的口径分档槽（口径=个体大小决定的鳃耙间距/口裂——@BhcSizeWindow 值域 [需正文]）
...
DECIDE_FIELD_FEEDING（分级命中，REP-ORDER-FIX-005 展开）：
    按三档判定 FieldIntakeEvaluation（档位成员=@BhcFieldIntakeProfile 值域不冻结）：
...
- 使用的自由度：census SINGLE 族 food_field 轴投影标签与场实例语义（plankton 场——census 冻结实例常量）；FOOD_FIELD_FEEDING_RESPONSE 族投影（canonical source）；intake_semantics 参数轴取值（持续滤食）；Profile 命名；伪脚本步序（canonical 两步固定；**§2.2/§3.2 已顺序还原（REP-ORDER-FIX-005）：链序与档位结构——推导依据 §0 判断顺序行**）。
- 放弃的自由度：(1) census canonical 步序的服从（顺序还原后链与 canonical 两步「无 gate」判语拓扑分歧——登记 README §7，裁决归 census 侧族重跑）；(2) 离散钩饵捕获通道表达（产品捕获方式 TAR-09 Open——census open_semantics 原样携带，不预购买）；(3) 合并算子（单场因子链无 combine 步；步间折减合成 OPERATOR UNDEFINED）；(4) 鱼群供给路由（census 判 NO_SURFACE_EFFECT，handoff 候选 Story DB 行级 [需核对]）；(5) 数值与 Profile 值域不冻结。
