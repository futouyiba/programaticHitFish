...
- 使用的自由度：census SINGLE 族 food_field 轴投影标签与场实例语义；FOOD_FIELD 族投影（intake_semantics=持续滤食）；Profile 命名；伪脚本步序（canonical 两步固定）；双批分工面划分（停食挂起归 P05 资产）。
- 放弃的自由度：溯河停食面表达（migration 批双 Path 资产——不重复）；离散钩饵捕获通道表达（产品捕获方式 TAR-09 Open——不预购买）；合并算子（单场因子链无 combine 步；OPERATOR UNDEFINED）；数值与 Profile 值域不冻结。
