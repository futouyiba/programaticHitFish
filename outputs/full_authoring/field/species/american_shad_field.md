# 美洲西鲱（American Shad｜Alosa sapidissima）｜滤食浮游场系四面生产级表达（P03 面，双批分工）
...
| Story | FISH-R06 停食第 2 例（原文「Feeding ceases during upstream spawning migration」；migration 批 american_shad.md 承载 P05 停食洄游双 Path 面）；本文件为其 **P03 摄食期面**——摄食期海域滤食（双批分工，[需正文]） |
| 冻结 Pattern | P05 停食洄游（migration 批承载）+ P03 摄食面（CSV 行矛盾+属级滤食方向 [需核对]——双面双批分工） |
| 物种属性锚 | fish-reference-20260908：pelagic-neritic、全天活跃、**肉食性名义行（hunting macrofauna）——与 Alosa 属滤食方向矛盾（登记 README §4 登记项 3）**、活泼、anadromous、营养级 3.48（行级 AI 审核状态=待人工审核；仅作方向锚） |
| 基线 | SNAPSHOT_ONLY（live Stress Test R1 主页转录 tmp/live_stress_main_after.md，2026-09-10 版；census registry v4 快照） |
| 证据档 | Tier B+（migration 批互指 + CSV 矛盾弱锚；Story 正文 [需正文]，条件值全 @ 化） |
...
- 双批分工互指：P05 停食洄游面（双 Path）＝migration 批 species/american_shad.md；P03 摄食面（本文件）。停食期=FieldFeeding 通道整体挂起（R06 停食判例「不再评价普通 Feeding」结构关闭同款语义在 Field 通道的对应——挂起表达归 migration 批双 Path 资产，本文件不重复）。
- CSV 行矛盾：食性=肉食性（hunting macrofauna）与 Alosa 属成体滤食方向冲突——方向锚取弱，场构成 [需正文]（README §4 登记项 3）。
...
- **判断顺序（REP-ORDER-FIX-005 顺序还原，handoff 判序+属级方向锚推导）**：判断链＝滤食水层定位（海域沿岸中上层滤食带三档——pelagic-neritic 方向锚+属级滤食方向 [需正文]）→ 场浓度判定（浮游浓度三档；水流输送调制并入 Profile 值域）→ 个体大小口径判定（鳃耙口径三档——CSV 行食性矛盾构成 [需正文]）→ 归一化（族常量终点步——SINGLE 族域无 combine 步）。推导来源＝handoff 判序指令（滤食型：先判水层位置（上层/中层）→水流/食物场密度→个体大小口径→合并）+ BHC canonical 同型骨架参数差异化（链形照 bighead_carp.md 顺序还原链，水层锚换海域沿岸中上层）；与 census canonical 两步判语的拓扑分歧登记 README §7。分级命中：每步三档（偏好=全额/可接受=削减×衰减不清零/不接受=出局 EARLY_RETURN）；early return 的对象=格子。档位成员与阈值全 Profile 值域不冻结 [需正文]。

Profile 引用清单：@AmsPlanktonFieldEvaluatorProfile @AmsPlanktonPreyFields @AmsDietClasses @AmsSizeWindow @AmsFieldIntakeProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| Ams_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| FieldType(typed) | food_field：plankton 场（场 evaluand——浮游浓度场，海域沿岸水柱分布（属级滤食方向锚；CSV 行食性矛盾——构成以 Story 正文为准 [需正文]）） |
| FieldEvaluatorProfile | @AmsPlanktonFieldEvaluatorProfile（场评估器：场事实→场评估器→场适应性） |
| FactorBinding | lifecycle premise：溯河洄游相位配置级切换（摄食期海域因子集 ↔ 溯河产卵期因子集由上游 premise 配置切换，不建 body 分支——MGC/CHB 先例） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@AmsPlanktonPreyFields 绑定的 plankton prey class 生物量——契约 FILTERED_SUM 聚合与原始事实族口径直接复用；diet_classes=@AmsDietClasses；size_window=@AmsSizeWindow） |
...
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@AmsPlanktonPreyFields 绑定的 plankton prey class 生物量，
      经 diet_classes=@AmsDietClasses 食性过滤
      与 size_window=@AmsSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）
读取 当前 premise（溯河洄游相位——lifecycle premise 配置级切换；不在本 body 内分支）

第 1 步 滤食水层定位（场评估的前置顺序判定，分级命中）：
    用水层/深度带事实查询 @AmsPlanktonFieldEvaluatorProfile 的水层分档槽
    （海域沿岸中上层滤食带——pelagic-neritic 方向锚+属级滤食方向 [需正文]；
      滤食型先判水层位置（上层/中层）；档位成员与阈值=Profile 值域不冻结 [需正文]）
    如果 ∈ 偏好滤食水层（preferred 槽——沿岸中上层浮游富集带）：
        不折减，进入第 2 步
...
第 2 步 EVAL_FOOD_FIELD_CONCENTRATION（canonical 步的档位化展开——场浓度三档）：
    用食物场浓度事实查询 @AmsPlanktonFieldEvaluatorProfile
...
第 3 步 个体大小口径判定（口径三档——契约口径过滤维度的链位化）：
    用口径过滤前后的构成对比（食物场浓度事实 vs diet_classes=@AmsDietClasses
      与 size_window=@AmsSizeWindow 过滤后的在场可食构成）查询 @AmsPlanktonFieldEvaluatorProfile
      的口径分档槽（口径=个体大小决定的鳃耙间距/口裂——CSV 行食性矛盾（肉食性行 vs
      属级滤食）构成以 Story 正文为准 [需正文]；@AmsSizeWindow 值域）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（FieldFeeding；intake_semantics=持续滤食） | @AmsFieldIntakeProfile | 返回 FieldFeedingResponse | 返回低 / 无响应 |
...
EVAL_FOOD_FIELD_INTAKE：
    用食物场浓度评价 @AmsFieldIntakeProfile
    （场摄入评估——持续滤食摄入语义（摄食期海域持续滤食；**溯河停食期 FieldFeeding 通道挂起**——停食形态由 migration 批双 Path 承载，本文件只表达摄食期）；
...
DECIDE_FIELD_FEEDING（分级命中，REP-ORDER-FIX-005 展开）：
    按三档判定 FieldIntakeEvaluation（档位成员=@AmsFieldIntakeProfile 值域不冻结）：
...
- 使用的自由度：census SINGLE 族 food_field 轴投影标签与场实例语义；FOOD_FIELD 族投影（intake_semantics=持续滤食）；Profile 命名；伪脚本步序（canonical 两步固定；**§2.2/§3.2 已顺序还原（REP-ORDER-FIX-005）：链序与档位结构（照 BHC 样板链，水层锚换海域沿岸中上层）——推导依据 §0 判断顺序行**）；双批分工面划分（停食挂起归 P05 资产）。
- 放弃的自由度：census canonical 步序的服从（顺序还原后链与 canonical 两步「无 gate」判语拓扑分歧——登记 README §7，裁决归 census 侧族重跑）；溯河停食面表达（migration 批双 Path 资产——不重复）；离散钩饵捕获通道表达（产品捕获方式 TAR-09 Open——不预购买）；合并算子（单场因子链无 combine 步；步间折减合成 OPERATOR UNDEFINED）；数值与 Profile 值域不冻结。
- [需正文] P03 主 relation 行级标签、摄食期场构成（CSV 矛盾消解）。
