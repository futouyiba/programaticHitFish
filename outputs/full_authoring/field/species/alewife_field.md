...
- **判断顺序（REP-ORDER-FIX-005 顺序还原，handoff 判序+属级方向锚推导）**：判断链＝滤食水层定位（海域沿岸中上层滤食带三档——pelagic-neritic 方向锚+属级滤食方向；湖封种群亦滤食 [需正文]）→ 场浓度判定（浮游浓度三档；水流输送调制并入 Profile 值域）→ 个体大小口径判定（鳃耙口径三档——CSV 行食性矛盾构成 [需正文]）→ 归一化（族常量终点步——SINGLE 族域无 combine 步）。推导来源＝handoff 判序指令（滤食型：先判水层位置（上层/中层）→水流/食物场密度→个体大小口径→合并）+ BHC canonical 同型骨架参数差异化（链形照 bighead_carp.md 顺序还原链，水层锚换海域沿岸中上层）；与 census canonical 两步判语的拓扑分歧登记 README §7。分级命中：每步三档（偏好=全额/可接受=削减×衰减不清零/不接受=出局 EARLY_RETURN）；early return 的对象=格子。档位成员与阈值全 Profile 值域不冻结 [需正文]。
...
第 1 步 滤食水层定位（场评估的前置顺序判定，分级命中）：
    用水层/深度带事实查询 @AlwPlanktonFieldEvaluatorProfile 的水层分档槽
    （海域沿岸中上层滤食带——pelagic-neritic 方向锚+属级滤食方向；
      湖封种群亦滤食 [需正文]；滤食型先判水层位置（上层/中层）；
...
第 3 步 个体大小口径判定（口径三档——契约口径过滤维度的链位化）：
    用口径过滤前后的构成对比（食物场浓度事实 vs diet_classes=@AlwDietClasses
      与 size_window=@AlwSizeWindow 过滤后的在场可食构成）查询 @AlwPlanktonFieldEvaluatorProfile
      的口径分档槽（口径=个体大小决定的鳃耙间距/口裂——CSV 行食性矛盾（肉食性行 vs
      属级滤食）构成以 Story 正文为准 [需正文]；@AlwSizeWindow 值域）
...
DECIDE_FIELD_FEEDING（分级命中，REP-ORDER-FIX-005 展开）：
    按三档判定 FieldIntakeEvaluation（档位成员=@AlwFieldIntakeProfile 值域不冻结）：
...
- 使用的自由度：census SINGLE 族 food_field 轴投影标签与场实例语义；FOOD_FIELD 族投影（intake_semantics=持续滤食）；Profile 命名；伪脚本步序（canonical 两步固定；**§2.2/§3.2 已顺序还原（REP-ORDER-FIX-005）：链序与档位结构（照 BHC 样板链，水层锚换海域沿岸中上层）——推导依据 §0 判断顺序行**）；双批分工面划分。
- 放弃的自由度：census canonical 步序的服从（顺序还原后链与 canonical 两步「无 gate」判语拓扑分歧——登记 README §7，裁决归 census 侧族重跑）；P05 洄游面表达（migration 批资产）；同属停食继承（不默认继承判语原样）；离散钩饵捕获通道表达（产品捕获方式 TAR-09 Open——不预购买）；合并算子（单场因子链无 combine 步；步间折减合成 OPERATOR UNDEFINED）；数值与 Profile 值域不冻结。
