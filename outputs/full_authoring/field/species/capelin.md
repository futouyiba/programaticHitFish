# 毛鳞鱼（Capelin｜Mallotus villosus）｜滤食浮游场系四面生产级表达
...
| Story | handoff 点名「鲱/西鲱/毛鳞鱼等浮游场系」；guarding/normal 批 K4 排除（繁殖/产卵锚聚类，无 guard relation、P01 归属无证据）——**该排除是繁殖面聚类口径**，本批按 P03 滤食面收录（CSV 滤食锚），繁殖产卵面归后续批不冒充（[需正文]） |
| 冻结 Pattern | P03 [需核对]（CSV 滤食锚 + handoff 点名；行级标签 Story DB 核对） |
| 物种属性锚 | fish-reference-20260908：pelagic-oceanic、全天活跃、滤食性（selective plankton feeding）、温和、anadromous、营养级 3.15（行级 AI 审核状态=待人工审核；仅作方向锚，数值不做阈值） |
| 基线 | SNAPSHOT_ONLY（live Stress Test R1 主页转录 tmp/live_stress_main_after.md，2026-09-10 版；census registry v4 快照） |
| 证据档 | Tier B（CSV 滤食锚 + handoff 点名先例——同 migration 批 atlantic_tarpon 档；Story 正文 [需正文]，条件值全 @ 化） |
...
- K4 排除口径澄清：guarding/normal 批对毛鳞鱼的排除依据是 K4 繁殖/产卵锚聚类（无 guard relation、P01 归属无证据）——排除的是其繁殖面在 P04/P01 批的冒充收录，不是其滤食面的 P03 归属。本文件按 CSV 滤食锚收录 P03 面（两面互不冒充；README §3 登记 5）。
...
- **判断顺序（REP-ORDER-FIX-005 顺序还原，handoff 判序+CSV 方向锚推导）**：判断链＝滤食水层定位（表层滤食带三档——北极/亚北极 pelagic-oceanic 表层带，CSV 方向锚 [需核对]）→ 场浓度判定（浮游浓度三档；水流输送调制并入 Profile 值域）→ 个体大小口径判定（鳃耙口径三档）→ 归一化（族常量终点步——SINGLE 族域无 combine 步）。推导来源＝handoff 判序指令（滤食型：先判水层位置（上层/中层）→水流/食物场密度→个体大小口径→合并）+ BHC canonical 同型骨架参数差异化（链形照 bighead_carp.md 顺序还原链，水层锚换表层）；与 census canonical 两步判语的拓扑分歧登记 README §7。分级命中：每步三档（偏好=全额/可接受=削减×衰减不清零/不接受=出局 EARLY_RETURN）；early return 的对象=格子。档位成员与阈值全 Profile 值域不冻结 [需正文]。

Profile 引用清单：@CapPlanktonFieldEvaluatorProfile @CapPlanktonPreyFields @CapDietClasses @CapSizeWindow @CapFieldIntakeProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| Cap_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| FieldType(typed) | food_field：plankton 场（场 evaluand——浮游浓度场，北极/亚北极表层水柱分布（CSV selective plankton feeding 方向锚）） |
| FieldEvaluatorProfile | @CapPlanktonFieldEvaluatorProfile（场评估器：场事实→场评估器→场适应性） |
| FactorBinding | lifecycle premise：溯河洄游相位配置级切换（摄食期海域因子集 ↔ 溯河产卵期因子集由上游 premise 配置切换，不建 body 分支——MGC/CHB 先例） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@CapPlanktonPreyFields 绑定的 plankton prey class 生物量——契约 FILTERED_SUM 聚合与原始事实族口径直接复用；diet_classes=@CapDietClasses；size_window=@CapSizeWindow） |
...
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@CapPlanktonPreyFields 绑定的 plankton prey class 生物量，
      经 diet_classes=@CapDietClasses 食性过滤
      与 size_window=@CapSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）
读取 当前 premise（溯河洄游相位——lifecycle premise 配置级切换；不在本 body 内分支）

第 1 步 滤食水层定位（场评估的前置顺序判定，分级命中）：
    用水层/深度带事实查询 @CapPlanktonFieldEvaluatorProfile 的水层分档槽
    （表层滤食带——北极/亚北极 pelagic-oceanic 表层带（CSV 方向锚 [需核对]）；
      滤食型先判水层位置（上层/中层）；档位成员与阈值=Profile 值域不冻结 [需正文]）
    如果 ∈ 偏好滤食水层（preferred 槽——表层浮游富集带）：
        不折减，进入第 2 步
...
第 2 步 EVAL_FOOD_FIELD_CONCENTRATION（canonical 步的档位化展开——场浓度三档）：
    用食物场浓度事实查询 @CapPlanktonFieldEvaluatorProfile
...
第 3 步 个体大小口径判定（口径三档——契约口径过滤维度的链位化）：
    用口径过滤前后的构成对比（食物场浓度事实 vs diet_classes=@CapDietClasses
      与 size_window=@CapSizeWindow 过滤后的在场可食构成）查询 @CapPlanktonFieldEvaluatorProfile
      的口径分档槽（口径=个体大小决定的鳃耙间距/口裂——@CapSizeWindow 值域 [需正文]）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（FieldFeeding；intake_semantics=持续滤食） | @CapFieldIntakeProfile | 返回 FieldFeedingResponse | 返回低 / 无响应 |
...
EVAL_FOOD_FIELD_INTAKE：
    用食物场浓度评价 @CapFieldIntakeProfile
    （场摄入评估——持续滤食摄入语义（摄食期持续滤食；产卵聚集期 K4 繁殖锚面不在本文件表达）；
...
DECIDE_FIELD_FEEDING（分级命中，REP-ORDER-FIX-005 展开）：
    按三档判定 FieldIntakeEvaluation（档位成员=@CapFieldIntakeProfile 值域不冻结）：
...
- 使用的自由度：census SINGLE 族 food_field 轴投影标签与场实例语义（plankton 场）；FOOD_FIELD 族投影（intake_semantics=持续滤食）；Profile 命名；伪脚本步序（canonical 两步固定；**§2.2/§3.2 已顺序还原（REP-ORDER-FIX-005）：链序与档位结构（照 BHC 样板链，水层锚换表层）——推导依据 §0 判断顺序行**）。
- 放弃的自由度：census canonical 步序的服从（顺序还原后链与 canonical 两步「无 gate」判语拓扑分歧——登记 README §7，裁决归 census 侧族重跑）；繁殖产卵面（K4 锚——归后续批；本文件不冒充）；离散钩饵捕获通道表达（产品捕获方式 TAR-09 Open——不预购买）；合并算子（单场因子链无 combine 步；步间折减合成 OPERATOR UNDEFINED）；数值与 Profile 值域不冻结。
- [需正文] P03 主 relation 行级标签、产卵期/摄食期相位细节。
