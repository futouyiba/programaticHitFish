...
- 使用的自由度：census SINGLE 族 food_field 轴投影标签与场实例语义（plankton 场）；FOOD_FIELD 族投影（intake_semantics=持续滤食）；Profile 命名；伪脚本步序（canonical 两步固定）。
- 放弃的自由度：繁殖产卵面（K4 锚——归后续批；本文件不冒充）；离散钩饵捕获通道表达（产品捕获方式 TAR-09 Open——不预购买）；合并算子（单场因子链无 combine 步；OPERATOR UNDEFINED）；数值与 Profile 值域不冻结。
