...
- **判断顺序（REP-ORDER-FIX-005 顺序还原，handoff 判序+方向锚推导）**：判断链＝场层位定位（饵鱼场所在层位三档——深槽/河道饵鱼群所在层，结构调制并入值域；晨昏水层切换 [需正文]）→ 场密度判定（饵鱼群密度三档）→ 个体大小口径判定（piscivore 口裂口径三档）→ 归一化（族常量终点步——SINGLE 族域无 combine 步）。推导来源＝handoff 判序指令（场跟随型：先判水层位置→水流/食物场密度→个体大小口径→合并——本鱼「水层位置」=饵鱼场所在层位非滤食水层）+ CSV 方向锚（晨昏活跃/深槽结构调制）；与 census canonical 两步判语的拓扑分歧登记 README §7。分级命中：每步三档（偏好=全额/可接受=削减×衰减不清零/不接受=出局 EARLY_RETURN）；early return 的对象=格子。档位成员与阈值全 Profile 值域不冻结 [需正文]。
...
第 1 步 场层位定位（场评估的前置顺序判定，分级命中）：
    用水层/深度带事实查询 @PrbBaitfishFieldEvaluatorProfile 的层位分档槽
    （饵鱼场所在层位——本鱼「水层位置」判定=饵鱼群所在层归属
      （深槽/河道结构对场分布的调制并入值域——配置表读法；晨昏水层切换 [需正文]）；
      档位成员与阈值=Profile 值域不冻结 [需正文]）
    如果 ∈ 偏好饵鱼场层位（preferred 槽——饵鱼群富集层位带）：
        不折减，进入第 2 步
    否则如果 ∈ 可接受邻层（tolerated 槽）：
        携削减标记进入第 2 步（× Profile 衰减参数——削减但不清零）
    否则（远离饵鱼场层位，场不在位）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；格子不在饵鱼场层位范围，出局）

第 2 步 EVAL_FOOD_FIELD_CONCENTRATION（canonical 步的档位化展开——饵鱼场密度三档）：
    用饵鱼场浓度事实查询 @PrbBaitfishFieldEvaluatorProfile
    （场适应性——场浓度评估，非离散 patch、非结构因子）
    如果 ∈ 高密度饵鱼群带（preferred 槽——饵鱼群密度在偏好带）：
        FieldSuitability = 全额
...
第 3 步 个体大小口径判定（口径三档——契约口径过滤维度的链位化）：
    用口径过滤前后的构成对比（饵鱼场浓度事实 vs diet_classes=@PrbDietClasses
      与 size_window=@PrbSizeWindow 过滤后的在场可食构成）查询 @PrbBaitfishFieldEvaluatorProfile
      的口径分档槽（口径=个体大小决定的 piscivore 口裂——@PrbSizeWindow 值域 [需正文]）
    如果 口径内构成为主（preferred 槽——场构成与本鱼口裂匹配）：
        FieldSuitability 保持（本步不折减）
    否则如果 仅部分构成在口径内（tolerated 槽）：
        FieldSuitability = 削减（× Profile 衰减参数叠加——部分可食）
    否则（场构成全部超出本鱼口裂——密度高亦不可食）：
...
DECIDE_FIELD_FEEDING（分级命中，REP-ORDER-FIX-005 展开）：
    按三档判定 FieldIntakeEvaluation（档位成员=@PrbFieldIntakeProfile 值域不冻结）：
    如果 FieldIntakeEvaluation ∈ 接受档（preferred 槽——饵鱼场密度足够启动脉冲捕获摄取）：
        返回 Response(FieldFeeding)（全额响应）
    否则如果 ∈ 边际档（tolerated 槽——稀薄饵鱼场仍维持低频跟随）：
        返回低响应（削减但不清零）
    否则：
        返回无响应（出局）
...
- 使用的自由度：FOOD_FIELD 族参数轴实例选取（field_type=饵鱼场、intake_semantics=脉冲摄入——两轴都是族声明参数轴非新结构）；Profile 命名；伪脚本步序（canonical 两步固定；**§2.2/§3.2 已顺序还原（REP-ORDER-FIX-005）：链序与档位结构——推导依据 §0 判断顺序行**）；BA-T5 参照读法选择。
- 放弃的自由度：(1) census canonical 步序的服从（顺序还原后链与 canonical 两步「无 gate」判语拓扑分歧——登记 README §7，裁决归 census 侧族重跑）；(2) piscivore 独立建组（CSV 行食性列空——piscivore 方向是属级方向锚，prey class 构成进 Profile 值域不建独立结构）；(3) 单次捕获 Encounter/Conversion 语义（捕获边界 owner——不在 Response 程序内折算）；(4) 离散钩饵捕获通道表达（TAR-09 Open）；(5) 合并算子（单场因子链无 combine 步；步间折减合成 OPERATOR UNDEFINED）；(6) 数值与 Profile 值域不冻结。
- [需正文] Story 行（R10 批 P03 主 relation 行级标签）、食性/场构成、晨昏水层切换（若有）。

BATCH_ID: REP-FULL-FIELD-001
顺序还原修复批次：REP-ORDER-FIX-005（§0/§2/§3/§5 修改；Bake 场层位定位→场密度→口径链+early return，Response 档位展开）
