# 湖白鲑（Lake Whitefish｜Coregonus artedi）｜滤食浮游场系四面生产级表达（P03 面，双批分工）
...
| Story | migration 批登记项 4「Whitefish 通名系（湖白鲑/驼背白鲑/高白鲑）CSV 行在但 P05 Story 零本地证据；归档后补批」——本批按 CSV 滤食锚收 **P03 摄食面**（P05 面仍零本地证据不冒充；[需正文]） |
| 冻结 Pattern | P03 [需核对]（CSV 滤食锚；行级标签 Story DB 核对；P05 面零本地证据不在本文件冒充） |
| 物种属性锚 | fish-reference-20260908：pelagic-neritic、晨昏活跃、滤食性（selective plankton feeding）、温和、anadromous 名义行、营养级 3.35（行级 AI 审核状态=待人工审核；仅作方向锚，数值不做阈值） |
| 基线 | SNAPSHOT_ONLY（live Stress Test R1 主页转录 tmp/live_stress_main_after.md，2026-09-10 版；census registry v4 快照） |
| 证据档 | Tier B（CSV 滤食锚；Story 正文 [需正文]，条件值全 @ 化） |
...
- Whitefish 通名系行级区分（inconnu.md §0 先例）：本行=Coregonus artedi（cisco 系湖白鲑），与驼背白鲑（C. pidschian）/高白鲑（C. peled）种级区分——通名系成员不互并。
...
- **判断顺序（REP-ORDER-FIX-005 顺序还原，handoff 判序+CSV 方向锚推导）**：判断链＝滤食水层定位（冷水湖泊中下层-水柱滤食带三档——白鲑湖栖深水带方向锚；湖底双壳/端足构成的水柱-近底摄食 [需正文]）→ 场浓度判定（浮游浓度三档；水流输送调制并入 Profile 值域）→ 个体大小口径判定（鳃耙口径三档）→ 归一化（族常量终点步——SINGLE 族域无 combine 步）。推导来源＝handoff 判序指令（滤食型：先判水层位置（上层/中层）→水流/食物场密度→个体大小口径→合并）+ BHC canonical 同型骨架参数差异化（链形照 bighead_carp.md 顺序还原链，水层锚换湖泊中下层-水柱带）；与 census canonical 两步判语的拓扑分歧登记 README §7。分级命中：每步三档（偏好=全额/可接受=削减×衰减不清零/不接受=出局 EARLY_RETURN）；early return 的对象=格子。档位成员与阈值全 Profile 值域不冻结 [需正文]。

Profile 引用清单：@LkwPlanktonFieldEvaluatorProfile @LkwPlanktonPreyFields @LkwDietClasses @LkwSizeWindow @LkwFieldIntakeProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| Lkw_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| FieldType(typed) | food_field：plankton 场（场 evaluand——浮游浓度场，冷水湖泊/沿岸水柱分布（CSV selective plankton feeding 方向锚；湖底双壳/端足构成 [需正文]）） |
| FieldEvaluatorProfile | @LkwPlanktonFieldEvaluatorProfile（场评估器：场事实→场评估器→场适应性） |
| FactorBinding | lifecycle premise：季节相位配置级切换（因子集随上游 premise 配置切换，不建 body 分支） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@LkwPlanktonPreyFields 绑定的 plankton prey class 生物量——契约 FILTERED_SUM 聚合与原始事实族口径直接复用；diet_classes=@LkwDietClasses；size_window=@LkwSizeWindow） |
...
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@LkwPlanktonPreyFields 绑定的 plankton prey class 生物量，
      经 diet_classes=@LkwDietClasses 食性过滤
      与 size_window=@LkwSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）
读取 当前 premise（季节相位——lifecycle premise 配置级切换；不在本 body 内分支）

第 1 步 滤食水层定位（场评估的前置顺序判定，分级命中）：
    用水层/深度带事实查询 @LkwPlanktonFieldEvaluatorProfile 的水层分档槽
    （冷水湖泊中下层-水柱滤食带——白鲑湖栖深水带方向锚；湖底双壳/端足构成的
      水柱-近底摄食 [需正文]；滤食型先判水层位置（上层/中层）；
      档位成员与阈值=Profile 值域不冻结 [需正文]）
    如果 ∈ 偏好滤食水层（preferred 槽——湖泊中下层浮游富集带）：
        不折减，进入第 2 步
...
第 3 步 个体大小口径判定（口径三档——契约口径过滤维度的链位化）：
    用口径过滤前后的构成对比（食物场浓度事实 vs diet_classes=@LkwDietClasses
      与 size_window=@LkwSizeWindow 过滤后的在场可食构成）查询 @LkwPlanktonFieldEvaluatorProfile
      的口径分档槽（口径=个体大小决定的鳃耙间距/口裂——湖底双壳/端足构成 [需正文]；
      @LkwSizeWindow 值域）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（FieldFeeding；intake_semantics=持续滤食） | @LkwFieldIntakeProfile | 返回 FieldFeedingResponse | 返回低 / 无响应 |
...
EVAL_FOOD_FIELD_INTAKE：
    用食物场浓度评价 @LkwFieldIntakeProfile
    （场摄入评估——持续滤食摄入语义（湖沼种群水层选择性并入值域——晨昏活跃方向锚）；
...
DECIDE_FIELD_FEEDING（分级命中，REP-ORDER-FIX-005 展开）：
    按三档判定 FieldIntakeEvaluation（档位成员=@LkwFieldIntakeProfile 值域不冻结）：
...
- 使用的自由度：census SINGLE 族 food_field 轴投影标签与场实例语义（plankton 场）；FOOD_FIELD 族投影（intake_semantics=持续滤食）；Profile 命名；伪脚本步序（canonical 两步固定；**§2.2/§3.2 已顺序还原（REP-ORDER-FIX-005）：链序与档位结构（照 BHC 样板链，水层锚换湖泊中下层-水柱带）——推导依据 §0 判断顺序行**）。
- 放弃的自由度：census canonical 步序的服从（顺序还原后链与 canonical 两步「无 gate」判语拓扑分歧——登记 README §7，裁决归 census 侧族重跑）；P05 洄游面（零本地证据不冒充——migration 批登记项 4 联动）；离散钩饵捕获通道表达（产品捕获方式 TAR-09 Open——不预购买）；合并算子（单场因子链无 combine 步；步间折减合成 OPERATOR UNDEFINED）；数值与 Profile 值域不冻结。
- [需正文] P03 主 relation 行级标签、场构成、通名系行级核对。
