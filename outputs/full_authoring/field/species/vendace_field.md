# 欧白鲑（Vendace｜Coregonus albula）｜滤食浮游场系四面生产级表达（P03 面，双批分工）
...
| Story | FISH-R02-S04 主 Story 已由 migration 批承载 P05 面（vendace.md：冷水湖泊底层/水层切换，census B2 快照）；本文件为其 **P03 摄食面**——migration 批 §3.2 判语「滤食浮游场语义若 Story 证实为场摄食，P03 域复核（FOOD_FIELD 族先例）」的兑现线（双批分工，P03 主 relation 行 [需核对]） |
| 冻结 Pattern | P05（census 冻结）+ P03 摄食面（CSV 滤食锚 [需核对]——P05+P03 双面双批分工） |
| 物种属性锚 | fish-reference-20260908：benthopelagic、晨昏活跃、滤食性（selective plankton feeding）、撕鳍、anadromous 名义行（湖封种群）、营养级 3.06（行级 AI 审核状态=待人工审核；仅作方向锚，数值不做阈值） |
| 基线 | SNAPSHOT_ONLY（live Stress Test R1 主页转录 tmp/live_stress_main_after.md，2026-09-10 版；census registry v4 快照） |
| 证据档 | Tier B+（CSV 滤食锚 + migration 批 P03 复核线点名；Story 正文 [需正文]，条件值全 @ 化） |
...
- 双批分工互指：P05 面（水层切换单链 BA-MIGRATION-SINGLE）＝migration 批 species/vendace.md；P03 面（本文件：plankton 场滤食 BA-P03-FIELD-SINGLE）。两文件互指不重复——P05 面归 migration 批资产不重写；本文件不承载洄游面。同种双面分工先例：白斑狗鱼三 Story 三文件三批。
...
- **判断顺序（REP-ORDER-FIX-005 顺序还原，handoff 判序+CSV 方向锚推导）**：判断链＝滤食水层定位（晨昏垂直迁移水层带三档——深水避光↔晨昏上层摄食的 DVM 方向（CSV 晨昏活跃锚+migration 批 P05 面水层 premise 同款参数方向 [需正文]）→ 场浓度判定（浮游浓度三档；水流输送调制并入 Profile 值域）→ 个体大小口径判定（鳃耙口径三档）→ 归一化（族常量终点步——SINGLE 族域无 combine 步）。推导来源＝handoff 判序指令（滤食型：先判水层位置（上层/中层）→水流/食物场密度→个体大小口径→合并）+ BHC canonical 同型骨架参数差异化（链形照 bighead_carp.md 顺序还原链，水层锚换晨昏迁移带）；与 census canonical 两步判语的拓扑分歧登记 README §7。分级命中：每步三档（偏好=全额/可接受=削减×衰减不清零/不接受=出局 EARLY_RETURN）；early return 的对象=格子。档位成员与阈值全 Profile 值域不冻结 [需正文]。

Profile 引用清单：@VdfPlanktonFieldEvaluatorProfile @VdfPlanktonPreyFields @VdfDietClasses @VdfSizeWindow @VdfFieldIntakeProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| Vdf_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| FieldType(typed) | food_field：plankton 场（场 evaluand——浮游浓度场，冷水湖泊水柱分布（CSV selective plankton feeding 方向锚；湖封种群冷水浮游构成）） |
| FieldEvaluatorProfile | @VdfPlanktonFieldEvaluatorProfile（场评估器：场事实→场评估器→场适应性） |
| FactorBinding | lifecycle premise：季节相位配置级切换（因子集随上游 premise 配置切换，不建 body 分支） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@VdfPlanktonPreyFields 绑定的 plankton prey class 生物量——契约 FILTERED_SUM 聚合与原始事实族口径直接复用；diet_classes=@VdfDietClasses；size_window=@VdfSizeWindow） |
...
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@VdfPlanktonPreyFields 绑定的 plankton prey class 生物量，
      经 diet_classes=@VdfDietClasses 食性过滤
      与 size_window=@VdfSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）
读取 当前 premise（季节相位——lifecycle premise 配置级切换；不在本 body 内分支）

第 1 步 滤食水层定位（场评估的前置顺序判定，分级命中）：
    用水层/深度带事实查询 @VdfPlanktonFieldEvaluatorProfile 的水层分档槽
    （晨昏垂直迁移水层带——深水避光↔晨昏上层摄食的 DVM 方向
      （CSV 晨昏活跃锚+migration 批 P05 面水层 premise 同款参数方向 [需正文]）；
      滤食型先判水层位置（上层/中层）；档位成员与阈值=Profile 值域不冻结 [需正文]）
    如果 ∈ 偏好滤食水层（preferred 槽——当前时段摄食水层带——晨昏上层/日间深水随相位取段）：
        不折减，进入第 2 步
    否则如果 ∈ 可接受邻层（tolerated 槽——迁移过渡带）：
        携削减标记进入第 2 步（× Profile 衰减参数——削减但不清零）
    否则（远离当前时段摄食水层，滤食位不在场）：
        返回 0（EARLY_RETURN：格子不在滤食水层范围，出局）

第 2 步 EVAL_FOOD_FIELD_CONCENTRATION（canonical 步的档位化展开——场浓度三档）：
    用食物场浓度事实查询 @VdfPlanktonFieldEvaluatorProfile
...
第 3 步 个体大小口径判定（口径三档——契约口径过滤维度的链位化）：
    用口径过滤前后的构成对比（食物场浓度事实 vs diet_classes=@VdfDietClasses
      与 size_window=@VdfSizeWindow 过滤后的在场可食构成）查询 @VdfPlanktonFieldEvaluatorProfile
      的口径分档槽（口径=个体大小决定的鳃耙间距/口裂——@VdfSizeWindow 值域 [需正文]）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（FieldFeeding；intake_semantics=持续滤食） | @VdfFieldIntakeProfile | 返回 FieldFeedingResponse | 返回低 / 无响应 |
...
EVAL_FOOD_FIELD_INTAKE：
    用食物场浓度评价 @VdfFieldIntakeProfile
    （场摄入评估——持续滤食摄入语义（冷水期底层/暖季水层的水层选择性并入接受窗值域——migration 批水层 premise 同款参数方向）；
...
DECIDE_FIELD_FEEDING（分级命中，REP-ORDER-FIX-005 展开）：
    按三档判定 FieldIntakeEvaluation（档位成员=@VdfFieldIntakeProfile 值域不冻结）：
...
- 使用的自由度：census SINGLE 族 food_field 轴投影标签与场实例语义（plankton 场——CSV 方向锚）；FOOD_FIELD_FEEDING_RESPONSE 族投影（intake_semantics=持续滤食）；Profile 命名；伪脚本步序（canonical 两步固定；**§2.2/§3.2 已顺序还原（REP-ORDER-FIX-005）：链序与档位结构（照 BHC 样板链，水层锚换晨昏迁移带）——推导依据 §0 判断顺序行**）；双批分工面划分（P05/P03 互指边界）。
- 放弃的自由度：census canonical 步序的服从（顺序还原后链与 canonical 两步「无 gate」判语拓扑分歧——登记 README §7，裁决归 census 侧族重跑）；P05 洄游面在本文件的表达（归 migration 批资产——双批分工不重复）；离散钩饵捕获通道表达（产品捕获方式 TAR-09 Open——不预购买）；合并算子（单场因子链无 combine 步；步间折减合成 OPERATOR UNDEFINED）；数值与 Profile 值域不冻结。
- [需核对] P03 主 relation 行级标签；anadromous 名义行与湖封种群口径（vendace.md §5 同款登记）。
