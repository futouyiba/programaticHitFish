# 旗鱼（Sailfish｜Istiophorus platypterus）｜追击型四面生产级表达

Status：WORKING / REPRESENTATION ARTIFACT / NOT AUTHORITY / NOT PROMOTED

| 项 | 值 |
|---|---|
| 批次 | REP-FULL-NORM2-001（P01 普通层＝全库最大 Pattern 群 第 6 批：第二轮全量闭合——追击组） |
| Story | FISH-R08 摘要点名（旗鱼喙击打 strike 变体；CSV 行 120） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：追猎、全天活跃、肉食性、营养级 4.5、pelagic-oceanic（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- Response 面：TYPED 族参数差异（高速拖饵/逃窜呈现（喙击打变体参数——R08 FR3 strike seam 判例）——组样板参数——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@SLFForageProfile @SLFHabitatProfile @SLFPreyFields @SLFDietClasses @SLFSizeWindow @SLFNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| SLF_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| Factor1Type(typed) | forage_factor：上层鱼群猎物场轴 |
| Factor2Type(typed) | habitat_factor：开阔大洋结构轴 |
| CombineRule | Template-fixed COMBINE_WEIGHTED（数学 OPERATOR UNDEFINED 待机制侧） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@SLFPreyFields；diet_classes=@SLFDietClasses；size_window=@SLFSizeWindow） |
...
读取 当前格子的forage_factor事实（上层鱼群猎物场轴）
读取 当前格子的habitat_factor事实（开阔大洋结构轴）
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@SLFPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@SLFDietClasses 食性过滤
      与 size_window=@SLFSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

槽 1 EVAL_TYPED_FIELD_OR_FACTOR（forage_factor，分级命中）：
    用forage_factor事实查询 @SLFForageProfile 的猎物场分档槽
    （上层鱼群猎物场轴；档位成员=Profile 值域不冻结 [需正文]）
...
槽 2 EVAL_TYPED_FIELD_OR_FACTOR（habitat_factor，分级命中）：
    用habitat_factor事实查询 @SLFHabitatProfile 的栖息分档槽
    （开阔大洋结构轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 适配档（preferred 槽）：
        OpenOceanHabitatFit = 全额
    否则如果 ∈ 过渡档（tolerated 槽）：
        OpenOceanHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（排除档）：
        OpenOceanHabitatFit = 出局槽值（excluded——非 EARLY_RETURN，同上族域边界）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @SLFNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（高速拖饵/逃窜呈现（喙击打变体参数——R08 FR3 strike seam 判例）——组样板参数）评价 @SLFNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@SLFNormalFeedingProfile 值域不冻结）：
...
- 放弃：喙击打变体的 Encounter/Conversion 边界表达（capture 机制归 owner 侧——本文件仅参数级注记）。
