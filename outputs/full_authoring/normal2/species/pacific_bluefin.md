# 太平洋蓝鳍金枪鱼（Pacific Bluefin Tuna｜Thunnus orientalis）｜追击型四面生产级表达
...
| Story | FISH-R08 摘要点名（Thunnus 4/4 真实；CSV 行 194） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：追猎、全天活跃、肉食性、营养级 —（源表空）、pelagic-oceanic（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
Profile 引用清单：@PBFForageProfile @PBFHabitatProfile @PBFPreyFields @PBFDietClasses @PBFSizeWindow @PBFNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| PBF_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| Factor1Type(typed) | forage_factor：跨洋鱼群猎物场轴 |
| Factor2Type(typed) | habitat_factor：温带开阔洋结构轴 |
| CombineRule | Template-fixed COMBINE_WEIGHTED（数学 OPERATOR UNDEFINED 待机制侧） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@PBFPreyFields；diet_classes=@PBFDietClasses；size_window=@PBFSizeWindow） |
...
读取 当前格子的forage_factor事实（跨洋鱼群猎物场轴）
读取 当前格子的habitat_factor事实（温带开阔洋结构轴）
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@PBFPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@PBFDietClasses 食性过滤
      与 size_window=@PBFSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

槽 1 EVAL_TYPED_FIELD_OR_FACTOR（forage_factor，分级命中）：
    用forage_factor事实查询 @PBFForageProfile 的猎物场分档槽
    （跨洋鱼群猎物场轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 猎物场 ∈ 丰档（preferred 槽）：
        OceanSchoolFit = 全额
    否则如果 ∈ 贫档（tolerated 槽）：
        OceanSchoolFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（无猎物档）：
        OceanSchoolFit = 出局槽值（excluded——非 EARLY_RETURN，出槽值仍进 COMBINE；
        PLAIN 族域内槽无 gate 语义）

槽 2 EVAL_TYPED_FIELD_OR_FACTOR（habitat_factor，分级命中）：
    用habitat_factor事实查询 @PBFHabitatProfile 的栖息分档槽
    （温带开阔洋结构轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 适配档（preferred 槽）：
        TemperateOceanHabitatFit = 全额
    否则如果 ∈ 过渡档（tolerated 槽）：
        TemperateOceanHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（排除档）：
        TemperateOceanHabitatFit = 出局槽值（excluded——非 EARLY_RETURN，同上族域边界）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @PBFNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（高速拖饵呈现——组样板参数）评价 @PBFNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@PBFNormalFeedingProfile 值域不冻结）：
