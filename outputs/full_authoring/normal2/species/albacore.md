# 长鳍金枪鱼（Albacore｜Thunnus alalunga）｜追击型四面生产级表达
...
| Story | FISH-R08 摘要点名（Thunnus 4/4 真实；CSV 行 210） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：追猎、全天活跃、肉食性、营养级 4.3、pelagic-oceanic（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- Response 面：TYPED 族参数差异（高速拖饵/水面呈现——组样板参数——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@ALBForageProfile @ALBHabitatProfile @ALBPreyFields @ALBDietClasses @ALBSizeWindow @ALBNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| ALB_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| Factor2Type(typed) | habitat_factor：副热带开阔洋结构轴 |
| CombineRule | Template-fixed COMBINE_WEIGHTED（数学 OPERATOR UNDEFINED 待机制侧） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@ALBPreyFields；diet_classes=@ALBDietClasses；size_window=@ALBSizeWindow） |
...
读取 当前格子的habitat_factor事实（副热带开阔洋结构轴）
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@ALBPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@ALBDietClasses 食性过滤
      与 size_window=@ALBSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

槽 1 EVAL_TYPED_FIELD_OR_FACTOR（forage_factor，分级命中）：
    用forage_factor事实查询 @ALBForageProfile 的猎物场分档槽
...
槽 2 EVAL_TYPED_FIELD_OR_FACTOR（habitat_factor，分级命中）：
    用habitat_factor事实查询 @ALBHabitatProfile 的栖息分档槽
    （副热带开阔洋结构轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 适配档（preferred 槽）：
        SubtropicOceanHabitatFit = 全额
    否则如果 ∈ 过渡档（tolerated 槽）：
        SubtropicOceanHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（排除档）：
        SubtropicOceanHabitatFit = 出局槽值（excluded——非 EARLY_RETURN，同上族域边界）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @ALBNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（高速拖饵/水面呈现——组样板参数）评价 @ALBNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@ALBNormalFeedingProfile 值域不冻结）：
