# 牛港鲹（Giant Trevally｜Caranx ignobilis）｜追击型四面生产级表达
...
| Story | R 系批成员（批归属未在本地 [需核对]；CSV 行 132） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：追猎、全天活跃、肉食性、营养级 4.22、reef-associated（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- Response 面：TYPED 族参数差异（大饵快速呈现/水面炸水呈现——组样板参数——组样板语义，参数级无新拓扑）。
...
|---|---|---|---|---|
| GTV_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| Bake 输入契约 | UsableForageAvailability（prey_fields=@GTVPreyFields；diet_classes=@GTVDietClasses；size_window=@GTVSizeWindow） |
...
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@GTVPreyFields 绑定的 prey class 生物量，
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @GTVNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
