# 短吻鲟（Shortnose Sturgeon｜Acipenser brevirostrum）｜伏击型四面生产级表达
...
| Story | FISH-R06 遗留点名（cookie/公开版拦截行；CSV 行 267） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：孤僻、晨昏活跃、肉食性、营养级 3.25、demersal（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- Response 面：TYPED 族参数差异（底栖贴近/小触饵呈现——组样板参数——组样板语义，参数级无新拓扑）。
- Group 面 / Quality 面：组样板 NO_SURFACE_EFFECT。
- 表达超集说明：骨架参数化表达；未超出组样板族域。

- **判断顺序（REP-ORDER-FIX-004 顺序还原，Tier B+）**：判断链＝GATE_ZONE 门（底层水层定位——小型鲟贴底特化，不成立=EARLY_RETURN） → 掩体结构档位（三档分级命中） → 归一化。水温/光照/时段等通用因子未入链（CSV 锚无 Story 空间程序证据——§5.1 例序是模板示例不是证据，不冒充）。

Profile 引用清单：@SNSRiverDeepBedProfile @SNSPreyFields @SNSDietClasses @SNSSizeWindow @SNSNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| SNS_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| FactorType(typed) | structure_factor：河底深槽/砾石贴近轴（小型鲟贴底方向） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@SNSPreyFields；diet_classes=@SNSDietClasses；size_window=@SNSSizeWindow） |
...
判断链＝GATE_ZONE 门（底层水层定位——小型鲟贴底特化） → 掩体结构档位 → 归一化；掩体档分级命中
...
读取 当前格子的structure_factor事实（河底深槽/砾石贴近轴（小型鲟贴底方向））
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@SNSPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@SNSDietClasses 食性过滤
      与 size_window=@SNSSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

第 1 步 GATE_ZONE（底层水层定位——小型鲟贴底特化——伏击型第一判断：无可用掩体不伏击）：
    用该 typed 因子轴事实查询 @SNSRiverDeepBedProfile 的掩体存在分档槽
...
第 2 步 掩体结构档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中）：
    用该 typed 因子轴事实查询 @SNSRiverDeepBedProfile 的掩体伏击分档槽
    （深槽/砾石贴近质量：槽底/砾石缘；档位成员=Profile 值域不冻结 [需正文]）
    如果 掩体结构 ∈ 最适伏击档（preferred 槽）：
        RiverDeepBedFit = 全额
    否则如果 ∈ 次级掩体档（tolerated 槽）：
        RiverDeepBedFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（暴露无掩体档）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；暴露格子不承载伏击分布）

第 3 步 NORMALIZE_WEIGHT：
    对 RiverDeepBedFit 执行模板固定归一化（族常量，非作者可选）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @SNSNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（底栖贴近/小触饵呈现——组样板参数）评价 @SNSNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@SNSNormalFeedingProfile 值域不冻结）：
