# 白亚口鱼（White Sucker｜Catostomus commersonii）｜伏击型四面生产级表达

Status：WORKING / REPRESENTATION ARTIFACT / NOT AUTHORITY / NOT PROMOTED

| 项 | 值 |
|---|---|
| 批次 | REP-FULL-NORM2-001（P01 普通层＝全库最大 Pattern 群 第 6 批：第二轮全量闭合——伏击组） |
| Story | REV-001 F1 跳行点名（REP-FULL-NORM2-001 handoff 转录；CSV 行 117） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：躲藏、早晨活跃、肉食性、营养级 2.82、demersal（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
| 亚结构组 | 伏击型（ambush/structure typed） |

## 0. 上游语义与摄食形态

- 摄食形态：白亚口鱼（砂砾底底栖贴近）。
- 证据边界：Story 行级 Pattern 标签不在本地 [需核对]；本文件结构为伏击组样板（第一轮 Tier A 样板骨架）的表达层选择——Story 正文到达后 census 判同可能改判（换组/换 BakeTemplate＝**结构变更需重审**，validator 族边界拦截静默改写），不是 Profile 重绑定。
- 附加注记：亚口科 P06 Pattern 标签张力（grazing 批登记项 1 对账线）+ CSV 英文名 Yaqui Sucker 与学名 C. commersonii（White Sucker）名实错位 [需核对]；handoff 点名单行收录——同科其余三行归 P06 对账线（README 排除表）。
- Response 面：TYPED 族参数差异（底栖贴近/吸贴呈现——组样板参数——组样板语义，参数级无新拓扑）。
- Group 面 / Quality 面：组样板 NO_SURFACE_EFFECT。
- 表达超集说明：骨架参数化表达；未超出组样板族域。

- **判断顺序（REP-ORDER-FIX-004 顺序还原，Tier B+）**：判断链＝GATE_ZONE 门（底层水层定位——底栖吸贴特化，不成立=EARLY_RETURN） → 掩体结构档位（三档分级命中） → 归一化。水温/光照/时段等通用因子未入链（CSV 锚无 Story 空间程序证据——§5.1 例序是模板示例不是证据，不冒充）。

Profile 引用清单：@WSCGravelBedProfile @WSCPreyFields @WSCDietClasses @WSCSizeWindow @WSCNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| WSC_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| FactorType(typed) | structure_factor：砂砾底/石缘贴近轴（底栖吸贴方向——CSV 肉食口径与亚口科底食形态张力 [需核对]） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@WSCPreyFields；diet_classes=@WSCDietClasses；size_window=@WSCSizeWindow） |
...
判断链＝GATE_ZONE 门（底层水层定位——底栖吸贴特化） → 掩体结构档位 → 归一化；掩体档分级命中
...
读取 当前格子的structure_factor事实（砂砾底/石缘贴近轴（底栖吸贴方向——CSV 肉食口径与亚口科底食形态张力 [需核对]））
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@WSCPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@WSCDietClasses 食性过滤
      与 size_window=@WSCSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

第 1 步 GATE_ZONE（底层水层定位——底栖吸贴特化——伏击型第一判断：无可用掩体不伏击）：
    用该 typed 因子轴事实查询 @WSCGravelBedProfile 的掩体存在分档槽
    （档位成员=Profile 值域不冻结 [需正文]）
    如果 格子位于底层水层带（砂砾底水层）：
        进入第 2 步
    否则：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；格子不在底层水层带——伏击型结构掩体先行判据）

第 2 步 掩体结构档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中）：
    用该 typed 因子轴事实查询 @WSCGravelBedProfile 的掩体伏击分档槽
    （砂砾底/石缘贴近质量：石缘吸贴/开阔砂底；档位成员=Profile 值域不冻结 [需正文]）
    如果 掩体结构 ∈ 最适伏击档（preferred 槽）：
        GravelBedFit = 全额
    否则如果 ∈ 次级掩体档（tolerated 槽）：
        GravelBedFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（暴露无掩体档）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；暴露格子不承载伏击分布）

第 3 步 NORMALIZE_WEIGHT：
    对 GravelBedFit 执行模板固定归一化（族常量，非作者可选）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @WSCNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（底栖贴近/吸贴呈现——组样板参数）评价 @WSCNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@WSCNormalFeedingProfile 值域不冻结）：
...
- 使用的自由度：SINGLE/PLAIN 族投影标签与 typed 因子实例语义；Profile 命名；顺序还原链序与档位结构（REP-ORDER-FIX-004：门先行 EARLY_RETURN+掩体档三档分级命中）。
...
- 放弃：亚口科族级归 P06/P01 裁决权（移交 Story DB 行级核对——张力登记不冒充）。
