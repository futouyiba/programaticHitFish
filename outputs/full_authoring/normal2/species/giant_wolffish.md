# 巨狼鱼（Giant Wolffish｜Hoplias aimara）｜追击型四面生产级表达
...
| Story | R 系批成员（批归属未在本地 [需核对]；CSV 行 179） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：追猎、晨昏活跃、肉食性、营养级 3.74、benthopelagic（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- 附加注记：名实注记——Hoplias aimara（erythrinid 巨狼鱼）≠ Anarhichas lupus（大西洋狼鱼，本批排除行 P04 对账线）；R1 候选池「巨狼鱼」即本行。
- Response 面：TYPED 族参数差异（大饵贴近/水面扰动呈现——组样板参数——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@GWFForageProfile @GWFHabitatProfile @GWFPreyFields @GWFDietClasses @GWFSizeWindow @GWFNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| GWF_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| Factor1Type(typed) | forage_factor：小鱼/无脊椎猎物场轴 |
| Factor2Type(typed) | habitat_factor：急流岩礁河道结构轴 |
| CombineRule | Template-fixed COMBINE_WEIGHTED（数学 OPERATOR UNDEFINED 待机制侧） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@GWFPreyFields；diet_classes=@GWFDietClasses；size_window=@GWFSizeWindow） |
...
读取 当前格子的forage_factor事实（小鱼/无脊椎猎物场轴）
读取 当前格子的habitat_factor事实（急流岩礁河道结构轴）
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@GWFPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@GWFDietClasses 食性过滤
      与 size_window=@GWFSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

槽 1 EVAL_TYPED_FIELD_OR_FACTOR（forage_factor，分级命中）：
    用forage_factor事实查询 @GWFForageProfile 的猎物场分档槽
    （小鱼/无脊椎猎物场轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 猎物场 ∈ 丰档（preferred 槽）：
        PreyFieldFit = 全额
    否则如果 ∈ 贫档（tolerated 槽）：
        PreyFieldFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（无猎物档）：
        PreyFieldFit = 出局槽值（excluded——非 EARLY_RETURN，出槽值仍进 COMBINE；
        PLAIN 族域内槽无 gate 语义）

槽 2 EVAL_TYPED_FIELD_OR_FACTOR（habitat_factor，分级命中）：
    用habitat_factor事实查询 @GWFHabitatProfile 的栖息分档槽
    （急流岩礁河道结构轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 适配档（preferred 槽）：
        RapidRockHabitatFit = 全额
    否则如果 ∈ 过渡档（tolerated 槽）：
        RapidRockHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（排除档）：
        RapidRockHabitatFit = 出局槽值（excluded——非 EARLY_RETURN，同上族域边界）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @GWFNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（大饵贴近/水面扰动呈现——组样板参数）评价 @GWFNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@GWFNormalFeedingProfile 值域不冻结）：
