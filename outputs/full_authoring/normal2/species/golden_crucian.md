# 黄金鲫（Golden Crucian｜Carassius auratus (hybrid)）｜机会组四面生产级表达
...
| Story | REV-001 F1 跳行点名（REP-FULL-NORM2-001 handoff 转录；CSV 行 3） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：活泼、早晨活跃、—（源表空）、营养级 —（源表空）、—（源表空）（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- 附加注记：Carassius 属对账注记——本行＝handoff 点名行；野生鲫鱼/金鲫/工程鲫行排除（属内种群/品系对账线——README 排除表）。
- Response 面：TYPED 族参数差异（小饵/粉饵机会呈现——组样板参数——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@GCRPatchProfile @GCRPreyFields @GCRDietClasses @GCRSizeWindow @GCRNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| GCR_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| FactorType(typed) | resource_patch：静水缓流杂食机会场轴（养殖杂交系——食性列空 [需正文]） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@GCRPreyFields；diet_classes=@GCRDietClasses；size_window=@GCRSizeWindow） |
...
（底板+光照槽）的顺序差异本身=LogicTemplate 判据。顺序来源＝批内互指/点名锚方向级推导（[需正文]）——Story 正文
到达后校准（census 侧 SINGLE 族重跑=work standards §5.4 行动项，分歧登记 README §7）。

读取 当前格子的resource_patch事实（静水缓流杂食机会场轴（养殖杂交系——食性列空 [需正文]））
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@GCRPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@GCRDietClasses 食性过滤
      与 size_window=@GCRSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

第 1 步 机会场食物丰度档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中——
  机会型第一判断：食物丰度先行）：
    用resource_patch事实查询 @GCRPatchProfile 的机会场丰度分档槽
    （静水缓流杂食机会场轴·养殖杂交系——食性列空 [需正文]；档位成员=Profile 值域不冻结 [需正文]）
    如果 机会场 ∈ 丰档（preferred 槽）：
        StillwaterPatchFit = 全额
    否则如果 ∈ 贫档（tolerated 槽）：
        StillwaterPatchFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（枯竭档——无可食机会）：
        返回 0（EARLY_RETURN：无食物机会的格子出局——机会型跟着食物走）

第 2 步 NORMALIZE_WEIGHT：
    对 StillwaterPatchFit 执行模板固定归一化（族常量，非作者可选）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @GCRNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（小饵/粉饵机会呈现——组样板参数）评价 @GCRNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@GCRNormalFeedingProfile 值域不冻结）：
...
- 放弃：属内身份裁决权（Carassius 属种群行归 Story DB 对账线）。
