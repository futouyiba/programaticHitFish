# 欧洲巨鲶（Wels Catfish｜Silurus glanis）｜夜行低光四面生产级表达
...
| Story | REV-001 F1 跳行点名（REP-FULL-NORM2-001 handoff 转录；CSV 行 35） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：孤僻、夜间活跃、肉食性、营养级 4.36、benthopelagic（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- 附加注记：双批分工——guarding 批 wels_catfish.md 承载 R06 P04 雄鱼洞巢守卵面；本文件＝CSV 夜行组合格行 P01 摄食面。
- Response 面：R-T1 Feeding-only + 光照/低光 Cue 参数（live §11.5 判例原样：RR-1 Feeding-only + LightAvailability / Cue Profile——低光不另开通道）；TYPED 族参数差异（夜相深水贴近/强振动呈现——组样板参数）。
...
Profile 引用清单：@WCFNightHabitatProfile @WCFLowLightSpatialProfile @WCFPreyFields @WCFDietClasses @WCFSizeWindow @WCFNormalFeedingProfile @WCFLightCueProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
| FactorType(typed) | habitat_factor：洞穴/深潭夜行底板轴（大型伏击鲶方向） |
| SpatialSlotProfile | @WCFLowLightSpatialProfile（低光空间槽——live §11.5 DynamicSpatialSlot 判例） |
...
| LiveLayerProjection | BA-T1 底板 + DynamicSpatialSlot=@WCFLowLightSpatialProfile（live §11.5 原样判例读法；两层 reconciliation OPEN——README §3） |
...
读取 当前格子的habitat_factor事实（洞穴/深潭夜行底板轴（大型伏击鲶方向））
...
第 1 步 夜行底板栖息档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中）：
    用habitat_factor事实查询 @WCFNightHabitatProfile 的夜行底板分档槽
    （洞穴/深潭夜行底板轴·大型伏击鲶方向；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 夜行底板档（preferred 槽）：
        BurrowNightHabitatFit = 全额
    否则如果 ∈ 次级夜栖档（tolerated 槽）：
        BurrowNightHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（无夜行底板档）：
        返回 0（EARLY_RETURN：无夜行底板的格子出局——夜行型栖息底板判据）

第 2 步 低光/夜相槽档位（APPLY_DYNAMIC_SPATIAL_SLOT 槽内三档——光照+时段判据；
  槽位置=live §11.5 判例固定原位，非作者可选）：
    用低光空间事实查询 @WCFLowLightSpatialProfile 的低光分档槽
...
第 3 步 NORMALIZE_WEIGHT：
    对 BurrowNightHabitatFit（含低光槽合入调整）执行模板固定归一化（族常量，非作者可选）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @WCFNormalFeedingProfile + @WCFLightCueProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（夜相深水贴近/强振动呈现——组样板参数）评价 @WCFNormalFeedingProfile
    得到 FoodEvaluation
