# 鲻鱼（Flathead Grey Mullet｜Mugil cephalus）｜机会组四面生产级表达
...
| Story | R 系批成员（批归属未在本地 [需核对]；CSV 行 254） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：活泼、早晨活跃、杂食性、营养级 2.48、benthopelagic（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- 附加注记：R1 候选池「鲻鱼」即本行（首轮裁决遗漏，本批补齐——README §1 声明）；CSV 杂食口径与刮食形态的 P06 边界张力登记在案。
- Response 面：TYPED 族参数差异（下沉饵/碎屑饵呈现——组样板参数——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@GMUPatchProfile @GMUPreyFields @GMUDietClasses @GMUSizeWindow @GMUNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| GMU_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| FactorType(typed) | resource_patch：河口泥底碎屑/藻屑机会场轴（CSV 杂食口径——现实底泥刮食倾向，P06 张力 [需核对]） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@GMUPreyFields；diet_classes=@GMUDietClasses；size_window=@GMUSizeWindow） |
...
读取 当前格子的resource_patch事实（河口泥底碎屑/藻屑机会场轴（CSV 杂食口径——现实底泥刮食倾向，P06 张力 [需核对]））
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@GMUPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@GMUDietClasses 食性过滤
      与 size_window=@GMUSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

第 1 步 机会场食物丰度档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中——
  机会型第一判断：食物丰度先行）：
    用resource_patch事实查询 @GMUPatchProfile 的机会场丰度分档槽
    （河口泥底碎屑/藻屑机会场轴·CSV 杂食口径——现实底泥刮食倾向，P06 张力 [需核对]；档位成员=Profile 值域不冻结 [需正文]）
    如果 机会场 ∈ 丰档（preferred 槽）：
        EstuaryDetritusPatchFit = 全额
    否则如果 ∈ 贫档（tolerated 槽）：
        EstuaryDetritusPatchFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（枯竭档——无可食机会）：
        返回 0（EARLY_RETURN：无食物机会的格子出局——机会型跟着食物走）

第 2 步 NORMALIZE_WEIGHT：
    对 EstuaryDetritusPatchFit 执行模板固定归一化（族常量，非作者可选）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @GMUNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（下沉饵/碎屑饵呈现——组样板参数）评价 @GMUNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@GMUNormalFeedingProfile 值域不冻结）：
