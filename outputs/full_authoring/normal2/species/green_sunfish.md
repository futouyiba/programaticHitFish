# 绿太阳鱼（Green Sunfish｜Lepomis cyanellus）｜机会组四面生产级表达
...
| Story | R 系批成员（批归属未在本地 [需核对]；CSV 行 116） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：好斗、晨昏活跃、肉食性、营养级 3.49、benthopelagic（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- 附加注记：太阳鱼系（驼背太阳鱼第一轮 R-T2 载体/蓝鳃 SIB guarding）互指——判例不继承；R1 候选池「绿太阳鱼」即本行。
- Response 面：TYPED 族参数差异（小饵/蠕虫呈现——组样板参数——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@GSNPatchProfile @GSNPreyFields @GSNDietClasses @GSNSizeWindow @GSNNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
| FactorType(typed) | resource_patch：浅水植被无脊椎机会场轴（好斗方向锚——同上统一 R-T1） |
...
读取 当前格子的resource_patch事实（浅水植被无脊椎机会场轴（好斗方向锚——同上统一 R-T1））
读取 当前格子的可食资源原始事实
...
第 1 步 机会场食物丰度档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中——
  机会型第一判断：食物丰度先行）：
    用resource_patch事实查询 @GSNPatchProfile 的机会场丰度分档槽
    （浅水植被无脊椎机会场轴·好斗方向锚——同上统一 R-T1；档位成员=Profile 值域不冻结 [需正文]）
    如果 机会场 ∈ 丰档（preferred 槽）：
        ShallowWeedPatchFit = 全额
    否则如果 ∈ 贫档（tolerated 槽）：
        ShallowWeedPatchFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（枯竭档——无可食机会）：
        返回 0（EARLY_RETURN：无食物机会的格子出局——机会型跟着食物走）

第 2 步 NORMALIZE_WEIGHT：
    对 ShallowWeedPatchFit 执行模板固定归一化（族常量，非作者可选）
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（小饵/蠕虫呈现——组样板参数）评价 @GSNNormalFeedingProfile
    得到 FoodEvaluation
