# 岩钝鲈（Rock Bass｜Ambloplites rupestris）｜机会组四面生产级表达
...
| Story | R 系批成员（批归属未在本地 [需核对]；CSV 行 134） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：好斗、晨昏活跃、肉食性、营养级 3.43、benthopelagic（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- 附加注记：太阳鱼科岩钝鲈属；R1 候选池「岩钝鲈」即本行。
- Response 面：TYPED 族参数差异（岩缝饵/活饵呈现——组样板参数——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@RKBPatchProfile @RKBPreyFields @RKBDietClasses @RKBSizeWindow @RKBNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| RKB_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| FactorType(typed) | resource_patch：岩礁结构小龙虾/小鱼机会场轴（好斗方向锚——同上统一 R-T1） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@RKBPreyFields；diet_classes=@RKBDietClasses；size_window=@RKBSizeWindow） |
...
读取 当前格子的resource_patch事实（岩礁结构小龙虾/小鱼机会场轴（好斗方向锚——同上统一 R-T1））
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@RKBPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@RKBDietClasses 食性过滤
      与 size_window=@RKBSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

第 1 步 机会场食物丰度档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中——
  机会型第一判断：食物丰度先行）：
    用resource_patch事实查询 @RKBPatchProfile 的机会场丰度分档槽
    （岩礁结构小龙虾/小鱼机会场轴·好斗方向锚——同上统一 R-T1；档位成员=Profile 值域不冻结 [需正文]）
    如果 机会场 ∈ 丰档（preferred 槽）：
        RockLedgePatchFit = 全额
    否则如果 ∈ 贫档（tolerated 槽）：
        RockLedgePatchFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（枯竭档——无可食机会）：
        返回 0（EARLY_RETURN：无食物机会的格子出局——机会型跟着食物走）

第 2 步 NORMALIZE_WEIGHT：
    对 RockLedgePatchFit 执行模板固定归一化（族常量，非作者可选）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @RKBNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（岩缝饵/活饵呈现——组样板参数）评价 @RKBNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@RKBNormalFeedingProfile 值域不冻结）：
