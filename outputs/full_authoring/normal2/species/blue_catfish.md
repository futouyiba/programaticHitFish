# 蓝鲶鱼（Blue Catfish｜Ictalurus furcatus）｜夜行低光四面生产级表达
...
| Story | R 系批成员（批归属未在本地 [需核对]；CSV 行 249） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：孤僻、夜间活跃、肉食性、营养级 3.41、demersal（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- 附加注记：同属斑点叉尾鮰（第一轮夜行组）互指——判例不继承。
- Response 面：R-T1 Feeding-only + 光照/低光 Cue 参数（live §11.5 判例原样：RR-1 Feeding-only + LightAvailability / Cue Profile——低光不另开通道）；TYPED 族参数差异（夜相深水贴近/振动呈现——组样板参数）。
...
Profile 引用清单：@BCTNightHabitatProfile @BCTLowLightSpatialProfile @BCTPreyFields @BCTDietClasses @BCTSizeWindow @BCTNormalFeedingProfile @BCTLightCueProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| BCT_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| FactorType(typed) | habitat_factor：大河道深水夜行底板轴（大型河道须系方向） |
| SpatialSlotProfile | @BCTLowLightSpatialProfile（低光空间槽——live §11.5 DynamicSpatialSlot 判例） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@BCTPreyFields；diet_classes=@BCTDietClasses；size_window=@BCTSizeWindow） |
| LiveLayerProjection | BA-T1 底板 + DynamicSpatialSlot=@BCTLowLightSpatialProfile（live §11.5 原样判例读法；两层 reconciliation OPEN——README §3） |
...
读取 当前格子的habitat_factor事实（大河道深水夜行底板轴（大型河道须系方向））
读取 当前格子的低光空间事实（低光/夜相空间槽——DynamicSpatialSlot）
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@BCTPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@BCTDietClasses 食性过滤
      与 size_window=@BCTSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

第 1 步 夜行底板栖息档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中）：
    用habitat_factor事实查询 @BCTNightHabitatProfile 的夜行底板分档槽
    （大河道深水夜行底板轴·大型河道须系方向；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 夜行底板档（preferred 槽）：
        BigRiverNightHabitatFit = 全额
    否则如果 ∈ 次级夜栖档（tolerated 槽）：
        BigRiverNightHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（无夜行底板档）：
        返回 0（EARLY_RETURN：无夜行底板的格子出局——夜行型栖息底板判据）

第 2 步 低光/夜相槽档位（APPLY_DYNAMIC_SPATIAL_SLOT 槽内三档——光照+时段判据；
  槽位置=live §11.5 判例固定原位，非作者可选）：
    用低光空间事实查询 @BCTLowLightSpatialProfile 的低光分档槽
...
第 3 步 NORMALIZE_WEIGHT：
    对 BigRiverNightHabitatFit（含低光槽合入调整）执行模板固定归一化（族常量，非作者可选）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @BCTNormalFeedingProfile + @BCTLightCueProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（夜相深水贴近/振动呈现——组样板参数）评价 @BCTNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@BCTNormalFeedingProfile 值域不冻结）：
