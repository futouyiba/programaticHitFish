# 双线无须鳕（Silver Hake｜Merluccius bilinearis）｜追击型四面生产级表达
...
| Story | R 系批成员（批归属未在本地 [需核对]；CSV 行 186） |
...
- Response 面：TYPED 族参数差异（中层拖饵/底层呈现——组样板参数——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@SLHForageProfile @SLHHabitatProfile @SLHPreyFields @SLHDietClasses @SLHSizeWindow @SLHNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| SLH_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| Factor2Type(typed) | habitat_factor：大陆架砂泥深水结构轴 |
| CombineRule | Template-fixed COMBINE_WEIGHTED（数学 OPERATOR UNDEFINED 待机制侧） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@SLHPreyFields；diet_classes=@SLHDietClasses；size_window=@SLHSizeWindow） |
...
读取 当前格子的habitat_factor事实（大陆架砂泥深水结构轴）
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@SLHPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@SLHDietClasses 食性过滤
      与 size_window=@SLHSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

槽 1 EVAL_TYPED_FIELD_OR_FACTOR（forage_factor，分级命中）：
    用forage_factor事实查询 @SLHForageProfile 的猎物场分档槽
    （底层鱼群猎物场轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 猎物场 ∈ 丰档（preferred 槽）：
        DemersalSchoolFit = 全额
    否则如果 ∈ 贫档（tolerated 槽）：
        DemersalSchoolFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（无猎物档）：
        DemersalSchoolFit = 出局槽值（excluded——非 EARLY_RETURN，出槽值仍进 COMBINE；
        PLAIN 族域内槽无 gate 语义）

槽 2 EVAL_TYPED_FIELD_OR_FACTOR（habitat_factor，分级命中）：
    用habitat_factor事实查询 @SLHHabitatProfile 的栖息分档槽
    （大陆架砂泥深水结构轴；档位成员=Profile 值域不冻结 [需正文]）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @SLHNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（中层拖饵/底层呈现——组样板参数）评价 @SLHNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@SLHNormalFeedingProfile 值域不冻结）：
