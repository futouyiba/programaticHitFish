# 大西洋小鳕（Atlantic Tomcod｜Microgadus tomcod）｜追击型四面生产级表达
...
| Story | R 系批成员（批归属未在本地 [需核对]；CSV 行 213） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：追猎、全天活跃、肉食性、营养级 3.81、demersal（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- 附加注记：鳕科（黑线鳕/绿青鳕第一轮、大西洋鳕鱼本批）互指——判例不继承；R1 候选池名单列名行（本批收——README §1 声明）。
- Response 面：TYPED 族参数差异（小饵贴底呈现——组样板参数——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@ATCForageProfile @ATCHabitatProfile @ATCPreyFields @ATCDietClasses @ATCSizeWindow @ATCNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| ATC_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| Factor1Type(typed) | forage_factor：底层无脊椎/小鱼猎物场轴 |
| Factor2Type(typed) | habitat_factor：河口浅水结构轴 |
| CombineRule | Template-fixed COMBINE_WEIGHTED（数学 OPERATOR UNDEFINED 待机制侧） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@ATCPreyFields；diet_classes=@ATCDietClasses；size_window=@ATCSizeWindow） |
...
读取 当前格子的forage_factor事实（底层无脊椎/小鱼猎物场轴）
读取 当前格子的habitat_factor事实（河口浅水结构轴）
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@ATCPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@ATCDietClasses 食性过滤
      与 size_window=@ATCSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

槽 1 EVAL_TYPED_FIELD_OR_FACTOR（forage_factor，分级命中）：
    用forage_factor事实查询 @ATCForageProfile 的猎物场分档槽
    （底层无脊椎/小鱼猎物场轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 猎物场 ∈ 丰档（preferred 槽）：
        EstuaryPreyFit = 全额
    否则如果 ∈ 贫档（tolerated 槽）：
        EstuaryPreyFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（无猎物档）：
        EstuaryPreyFit = 出局槽值（excluded——非 EARLY_RETURN，出槽值仍进 COMBINE；
        PLAIN 族域内槽无 gate 语义）

槽 2 EVAL_TYPED_FIELD_OR_FACTOR（habitat_factor，分级命中）：
    用habitat_factor事实查询 @ATCHabitatProfile 的栖息分档槽
    （河口浅水结构轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 适配档（preferred 槽）：
        ShallowEstuaryHabitatFit = 全额
    否则如果 ∈ 过渡档（tolerated 槽）：
        ShallowEstuaryHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（排除档）：
        ShallowEstuaryHabitatFit = 出局槽值（excluded——非 EARLY_RETURN，同上族域边界）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @ATCNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（小饵贴底呈现——组样板参数）评价 @ATCNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@ATCNormalFeedingProfile 值域不冻结）：
