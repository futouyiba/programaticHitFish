...
| 物种属性锚 | fish-reference-20260908：追猎、全天活跃、肉食性、营养级 4.09、benthopelagic（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- Response 面：TYPED 族参数差异（底层大饵/慢速拖行呈现——组样板参数——组样板语义，参数级无新拓扑）。
...
读取 当前格子的forage_factor事实（底层鱼类猎物场轴（鳕科群游追击方向））
读取 当前格子的habitat_factor事实（砂泥大陆架深水结构轴）
读取 当前格子的可食资源原始事实
...
槽 1 EVAL_TYPED_FIELD_OR_FACTOR（forage_factor，分级命中）：
    用forage_factor事实查询 @ACFForageProfile 的猎物场分档槽
    （底层鱼类猎物场轴·鳕科群游追击方向；档位成员=Profile 值域不冻结 [需正文]）
...
槽 2 EVAL_TYPED_FIELD_OR_FACTOR（habitat_factor，分级命中）：
    用habitat_factor事实查询 @ACFHabitatProfile 的栖息分档槽
    （砂泥大陆架深水结构轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 适配档（preferred 槽）：
        ShelfGroundHabitatFit = 全额
    否则如果 ∈ 过渡档（tolerated 槽）：
        ShelfGroundHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（排除档）：
        ShelfGroundHabitatFit = 出局槽值（excluded——非 EARLY_RETURN，同上族域边界）
