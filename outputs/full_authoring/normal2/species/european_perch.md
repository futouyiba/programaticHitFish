# 赤梢鱼（European Perch｜Perca fluviatilis）｜追击型四面生产级表达
...
| Story | R 系批成员（批归属未在本地 [需核对]；CSV 行 205） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：追猎、晨昏活跃、肉食性、营养级 4.35、demersal（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- 附加注记：CSV 中文名赤梢鱼，学名 Perca fluviatilis（欧洲河鲈）[需核对]；同属黄鲈（第一轮追击组）互指——判例不继承。
- Response 面：TYPED 族参数差异（小饵缓速呈现——组样板参数——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@EUPForageProfile @EUPHabitatProfile @EUPPreyFields @EUPDietClasses @EUPSizeWindow @EUPNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| EUP_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| Factor2Type(typed) | habitat_factor：植被结构湖泊轴 |
| CombineRule | Template-fixed COMBINE_WEIGHTED（数学 OPERATOR UNDEFINED 待机制侧） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@EUPPreyFields；diet_classes=@EUPDietClasses；size_window=@EUPSizeWindow） |
...
读取 当前格子的habitat_factor事实（植被结构湖泊轴）
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@EUPPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@EUPDietClasses 食性过滤
      与 size_window=@EUPSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

槽 1 EVAL_TYPED_FIELD_OR_FACTOR（forage_factor，分级命中）：
    用forage_factor事实查询 @EUPForageProfile 的猎物场分档槽
...
槽 2 EVAL_TYPED_FIELD_OR_FACTOR（habitat_factor，分级命中）：
    用habitat_factor事实查询 @EUPHabitatProfile 的栖息分档槽
    （植被结构湖泊轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 适配档（preferred 槽）：
        WeedyLakeHabitatFit = 全额
    否则如果 ∈ 过渡档（tolerated 槽）：
        WeedyLakeHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（排除档）：
        WeedyLakeHabitatFit = 出局槽值（excluded——非 EARLY_RETURN，同上族域边界）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @EUPNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（小饵缓速呈现——组样板参数）评价 @EUPNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@EUPNormalFeedingProfile 值域不冻结）：
