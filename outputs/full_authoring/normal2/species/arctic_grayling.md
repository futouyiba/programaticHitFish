# 北极茴鱼（Arctic Grayling｜Thymallus arcticus）｜伏击型四面生产级表达
...
| Story | R 系批成员（批归属未在本地 [需核对]；CSV 行 225） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：孤僻、晨昏活跃、肉食性、营养级 3.26、benthopelagic（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- 附加注记：鲑科非洄游标注行（CSV 迁徙类型列空）——不触发鲑系洄游 P05 对账排除（README 判据声明）。
- Response 面：TYPED 族参数差异（漂流呈现/干蝇水面呈现——组样板参数——组样板语义，参数级无新拓扑）。
- Group 面 / Quality 面：组样板 NO_SURFACE_EFFECT。
- 表达超集说明：骨架参数化表达；未超出组样板族域。

- **判断顺序（REP-ORDER-FIX-004 顺序还原，Tier B）**：判断链＝GATE_RIFFLE_GRAVEL 门（急流砾石结构存在，不成立=EARLY_RETURN） → 掩体结构档位（三档分级命中） → 归一化。水温/光照/时段等通用因子未入链（CSV 锚无 Story 空间程序证据——§5.1 例序是模板示例不是证据，不冒充）。

Profile 引用清单：@AGYRiffleGravelProfile @AGYPreyFields @AGYDietClasses @AGYSizeWindow @AGYNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| AGY_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
第 1 步 GATE_RIFFLE_GRAVEL（急流砾石结构存在——伏击型第一判断：无可用掩体不伏击）：
    ...
    如果 格子存在急流砾石/卵石滩结构：
        进入第 2 步
    否则：
        返回 0（EARLY_RETURN：格子无砾石结构——伏击型结构掩体先行判据）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @AGYNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（漂流呈现/干蝇水面呈现——组样板参数）评价 @AGYNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@AGYNormalFeedingProfile 值域不冻结）：
