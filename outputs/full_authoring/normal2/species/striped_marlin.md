# 条纹四鳍旗鱼（Striped Marlin｜Kajikia audax）｜追击型四面生产级表达
...
| Story | R 系批成员（批归属未在本地 [需核对]；CSV 行 140） |
...
- Response 面：TYPED 族参数差异（高速拖饵/水面逃窜呈现——组样板参数——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@STMForageProfile @STMHabitatProfile @STMPreyFields @STMDietClasses @STMSizeWindow @STMNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| STM_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| Bake 输入契约 | UsableForageAvailability（prey_fields=@STMPreyFields；diet_classes=@STMDietClasses；size_window=@STMSizeWindow） |
...
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@STMPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@STMDietClasses 食性过滤
      与 size_window=@STMSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

槽 1 EVAL_TYPED_FIELD_OR_FACTOR（forage_factor，分级命中）：
    用forage_factor事实查询 @STMForageProfile 的猎物场分档槽
...
槽 2 EVAL_TYPED_FIELD_OR_FACTOR（habitat_factor，分级命中）：
    用habitat_factor事实查询 @STMHabitatProfile 的栖息分档槽
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @STMNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（高速拖饵/水面逃窜呈现——组样板参数）评价 @STMNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@STMNormalFeedingProfile 值域不冻结）：
