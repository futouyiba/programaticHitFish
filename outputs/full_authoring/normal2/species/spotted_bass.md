# 斑点黑鲈（Spotted Bass｜Micropterus punctulatus）｜机会组四面生产级表达
...
| Story | R 系批成员（批归属未在本地 [需核对]；CSV 行 102） |
| 冻结 Pattern | P01 [需核对]（CSV 方向锚归层） |
| 物种属性锚 | fish-reference-20260908：好斗、晨昏活跃、肉食性、营养级 3.56、demersal（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- 附加注记：黑鲈系——小口黑鲈（guarding 批 P04）互指、大口黑鲈（Story DB 0 Story 隔离行）不继承；R1 候选池「斑点黑鲈」即本行。
- Response 面：TYPED 族参数差异（结构饵/软饵呈现——组样板参数——组样板语义，参数级无新拓扑）。
...
|---|---|---|---|---|
| SPB_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@SPBPreyFields 绑定的 prey class 生物量，
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @SPBNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
