...
第 1 步 斑块存在性（GATE_PATCH_PRESENCE）：
    ...
    如果 本格无植食资源斑块且无预投饵斑块（excluded 槽——零斑块）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；无斑块格不参与斑块追随分布）
    否则：
        进入第 2 步
...
第 2 步 斑块质量档位（EVAL_TYPED_FIELD_OR_FACTOR，分级命中）：
    ...
    否则（斑块存在但质量近零档）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；质量排除档出局）
