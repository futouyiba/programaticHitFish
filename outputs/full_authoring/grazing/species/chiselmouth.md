...
第 1 步 底层水层定位（GATE_ZONE——demersal 底栖特化硬定位）：
    ...
    如果 水层 ∈ 底层带档：
        进入第 2 步
    否则：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；demersal 表面刮食定位不在非底层分布——
...
第 2 步 硬基质档位（分级命中——附着面可得性）：
    ...
    否则（软底无附着面档）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；无附着刮取面的底质出局）
