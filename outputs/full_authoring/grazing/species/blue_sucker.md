# 长背亚口鱼（Blue Sucker｜Cycleptus elongatus）｜Grazing 系四面生产级表达
...
- 底质处理形态（方向）：亚口科底质连续吸食——底质表面/间隙无脊椎与有机资源的吸食处理（CSV benthopelagic、亚口科 Catostomidae 吸口型底栖取食体构方向；急流砾石底 vs 缓流软底偏好 [需正文]）。
...
|---|---|---|---|---|
| BlueSucker_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| Bake 输入契约 | UsableForageAvailability（prey_fields=@BlueSuckerSubstratePreyFields；diet_classes=@BlueSuckerDietClasses；size_window=@BlueSuckerSizeWindow） |
| LiveLayerProjection | BA-NORMAL-HABITAT-FIT + DynamicSpatialSlot=@BlueSuckerSubstrateSpatialProfile（coverage delta K3 吸收读法：benthic prey class + 基质 Factor；两层 reconciliation OPEN——README §3 登记） |
...
读取 当前格子的基质资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@BlueSuckerSubstratePreyFields 绑定的底栖无脊椎 prey class 生物量，
      经 diet_classes=@BlueSuckerDietClasses 食性过滤
      与 size_window=@BlueSuckerSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）
...
第 1 步 近底带水层定位（分级命中，软定位——CSV benthopelagic＋吸口底栖体构注记）：
    ...
    否则（远离底带档）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；吸食底栖取食定位不在远底水层分布——
...
第 2 步 底质栖境档位（分级命中——表面/间隙无脊椎栖境可得性）：
    ...
    否则（无栖境档——无脊椎不可栖底质）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；无可吸食栖境的底质出局）
...
第 3 步 无脊椎资源档位（EVAL_TYPED_FIELD_OR_FACTOR，分级命中）：
    ...
    否则（无资源档）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；无底栖无脊椎的格子出局）
...
live 侧吸收读法＝BA-T1 底板 + DynamicSpatialSlot=@BlueSuckerSubstrateSpatialProfile（REP-COVERAGE-DELTA-001 K3 吸收结构：benthic prey class + 基质 Factor）。census SINGLE 族与 live 句型层 reconciliation OPEN（README §3 登记 1）。
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @BlueSuckerNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
