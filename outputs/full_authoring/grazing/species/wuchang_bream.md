...
第 1 步 近底带水层定位（分级命中，软定位——CSV benthopelagic 底中两层）：
    ...
    否则（远离底带档）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；沉水植物啃食定位不在远底水层分布）
...
第 2 步 水草床构成档位（分级命中——SRCHECK「不毁草」条件化直接支持）：
    ...
    否则（无草床档）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；无沉水植物草床的格子出局）
...
第 3 步 啃食资源档位（EVAL_TYPED_FIELD_OR_FACTOR，分级命中）：
    ...
    否则（无可啃食资源档）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；无啃食资源的格子出局）
