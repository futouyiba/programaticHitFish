...
- **判断顺序（REP-ORDER-FIX-001 顺序还原，CSV 方向级推导 [需正文]）**：判断链＝底层水层定位 → 深度带档位 → 底泥栖境档位 → 底泥资源档位 → 归一化。推导来源：CSV 栖息带 demersal（硬定位：非底层=出局）＋**深≥4m（本鱼独有的深度带档步——四亚口中仅本鱼 CSV 有深度注记，链形因此与同批三鱼不同）**＋肉食性 hunting macrofauna（亚口科语境=底栖大型无脊椎吸食摄取）。分级命中：各步三档（最适应=全额/可接受=削减不清零/排除=出局），档位成员与阈值全 Profile 值域不冻结 [需正文]。CSV 时段（早晨活跃）/水温锚未入链。
...
判断链＝底层水层定位 → 深度带档位 → 底泥栖境档位 → 底泥资源档位 → 归一化；
每步分级命中（全额/削减/出局），出局即 EARLY_RETURN。深度带档步为本鱼独有
（CSV 深≥4m 注记——四亚口中仅本鱼有深度锚，链形与同批三鱼不同：
顺序差异本身=LogicTemplate 判据）。顺序为 CSV 方向级推导（[需正文]），
...
第 1 步 底层水层定位（GATE_ZONE——demersal 底栖特化硬定位）：
    用水层带位置查询 @BuffaloSubstrateSpatialProfile 的水层分档槽
    （档位成员=Profile 值域不冻结 [需正文]）
    如果 水层 ∈ 底层带档：
        进入第 2 步
    否则：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；demersal 底泥吸食定位不在非底层分布——
        底栖特化硬判定；若正文证实会离底取食则档位化=结构变更需重审）

第 2 步 深度带档位（分级命中——CSV 深≥4m 方向锚，本鱼独有步）：
    用深度事实查询 @BuffaloSubstrateSpatialProfile 的深度分档槽
    （偏好深度带成员与阈值=Profile 值域不冻结 [需正文]）
    如果 深度 ∈ 偏好深带档（preferred 槽）：
        DepthTier = 全额保留
    否则如果 深度 ∈ 过渡深度档（tolerated 槽）：
        DepthTier = 削减（× Profile 衰减参数——削减但不清零）
    否则（过浅带档）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；浅带格出局——深水底泥取食定位）

第 3 步 底泥栖境档位（分级命中——底泥无脊椎栖境可得性）：
    用底质类型查询 @BuffaloSubstratePatchProfile 的底质分档槽
    （底泥/淤泥承载方向；档位成员=Profile 值域不冻结 [需正文]）
    如果 底质 ∈ 底泥栖境档（preferred 槽——有机泥/淤泥栖境充分）：
        SubstrateTier = 全额保留
    否则如果 底质 ∈ 有限栖境档（tolerated 槽——如砂质混泥）：
        SubstrateTier = 削减（削减但不清零）
    否则（无栖境档——如硬砾无底泥）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；无底泥栖境的底质出局）

第 4 步 底泥资源档位（EVAL_TYPED_FIELD_OR_FACTOR，分级命中）：
    用基质资源事实查询 @BuffaloSubstratePatchProfile
    （单 typed 因子评估展开为三档分档槽=Profile 值域不冻结 [需正文]）
    如果 底泥无脊椎可得性 ∈ 丰档（preferred 槽）：
        SubstratePatchIntensity = 全额强度
    否则如果 ∈ 贫档（tolerated 槽）：
        SubstratePatchIntensity = 削减强度（削减但不清零）
    否则（无资源档）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；无底泥无脊椎的格子出局）

第 5 步 NORMALIZE_WEIGHT：
    对 DepthTier × SubstrateTier × SubstratePatchIntensity 执行模板固定归一化
    （族常量，非作者可选）
...
DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-001 展开）：
    按三档判定 FoodEvaluation（档位成员=@BuffaloNormalFeedingProfile 值域不冻结）：
...
- 使用的自由度：SINGLE 族投影标签；typed factor 实例语义（底泥无脊椎方向，取自 CSV）；Profile 命名；**顺序还原链序与档位结构（REP-ORDER-FIX-001：demersal 硬定位先行、深度带档步（本鱼独有，CSV 深≥4m 锚）、栖境/资源三档分级命中、early return 链——CSV 方向级推导 [需正文]）**；Bake 输入契约字段复用。
- 放弃的自由度：(1) 归族裁决权（同泰鲮）；(2) census canonical 步序的服从（顺序还原后链与 canonical 两步「无 gate 判语」拓扑分歧——登记 README §7，裁决归 census 侧族重跑）；(3) 合并算子（本程序无 combine 步；live 层组合算子 OPERATOR UNDEFINED 待机制侧）；(4) 与大口水牛鱼（C10 FieldFeeding）的合并表达（种级区分：Ictiobus bubalus ≠ I. cyprinellus，机制域 P06 底质吸食 ≠ P03 滤食场）；(5) 「孤僻」性格程序化（档案字段无 Story 证据映射，不表达）；(6) 水温/时段因子入链（CSV 锚无 Story 空间程序证据）；(7) 数值与 Profile 值域不冻结（含深度带与档位成员、阈值）。
- [需正文] 底泥无脊椎构成、深度带偏好参数（CSV 深≥4m 方向）、判断顺序与档位成员校准、premise 切换（若有）、Response 接受窗参数方向。

BATCH_ID: REP-FULL-GRAZE-001
顺序还原修复批次：REP-ORDER-FIX-001（§0/§2/§3/§5 修改；Bake 伪脚本 early return 链+分级命中（含深度带档步），Response 档位展开）
