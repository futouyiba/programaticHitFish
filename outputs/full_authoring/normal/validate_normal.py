#!/usr/bin/env python3
# REP-FULL-NORM-001 structural validator for P01 Discrete-Target (普通离散
# 目标摄食系) four-surface authoring files. Pure standard library. Adapted
# from validate_migration.py / validate_grazing.py with batch-specific
# checkpoints:
...
def main():
    ...
    if len(sys.argv) < 2:
        print("usage: validate_normal.py <normal_dir> | --selftest")
        sys.exit(2)
...
| 项 | 值 |
|---|---|
| 批次 | REP-FULL-NORM-001（P01 普通层＝全库最大 Pattern 群 第 4 批） |
| 基线 | SNAPSHOT_ONLY |
...
def run_selftest():
    ...
    case("HEADER fires on missing BATCH_ID line", lambda s: s.replace("BATCH_ID: REP-FULL-NORM-001", "BATCH_ID: other"), "HEADER")
