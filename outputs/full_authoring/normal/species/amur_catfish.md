# 土鲶（Amur Catfish｜Silurus asotus）｜夜行低光四面生产级表达
...
| 物种属性锚 | fish-reference-20260908：孤僻、夜间活跃、肉食性、营养级 4.43（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
Profile 引用清单：@AcfSlackBurrowNightHabitatProfile @AcfLowLightSpatialProfile @AcfPreyFields @AcfDietClasses @AcfSizeWindow @AcfNormalFeedingProfile @AcfLightCueProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
| FactorType(typed) | habitat_factor：缓流泥底/洞穴夜行底板轴 |
| SpatialSlotProfile | @AcfLowLightSpatialProfile（低光空间槽——live §11.5 DynamicSpatialSlot 判例） |
...
| LiveLayerProjection | BA-T1 底板 + DynamicSpatialSlot=@AcfLowLightSpatialProfile（live §11.5 原样判例读法；两层 reconciliation OPEN——README §3） |
...
读取 当前格子的habitat_factor事实（缓流泥底/洞穴夜行底板轴）
...
第 1 步 夜行底板栖息档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中）：
    用habitat_factor事实查询 @AcfSlackBurrowNightHabitatProfile 的夜行底板分档槽
    （缓流泥底/洞穴夜行底板轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 夜行底板档（preferred 槽）：
        SlackBurrowNightHabitatFit = 全额
    否则如果 ∈ 次级夜栖档（tolerated 槽）：
        SlackBurrowNightHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（无夜行底板档）：
        返回 0（EARLY_RETURN：无夜行底板的格子出局——夜行型栖息底板判据）

第 2 步 低光/夜相槽档位（APPLY_DYNAMIC_SPATIAL_SLOT 槽内三档——光照+时段判据；
  槽位置=live §11.5 判例固定原位，非作者可选）：
    用低光空间事实查询 @AcfLowLightSpatialProfile 的低光分档槽
...
第 3 步 NORMALIZE_WEIGHT：
    对 SlackBurrowNightHabitatFit（含低光槽合入调整）执行模板固定归一化（族常量，非作者可选）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @AcfNormalFeedingProfile + @AcfLightCueProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（夜相底层贴近/振动呈现——组样板参数）评价 @AcfNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@AcfNormalFeedingProfile 值域不冻结）：
