# 美洲鳗鲡（American Eel｜Anguilla rostrata）｜夜行低光四面生产级表达
...
| 物种属性锚 | fish-reference-20260908：孤僻、夜间活跃、肉食性、营养级 3.83（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- Response 面：R-T1 Feeding-only + 光照/低光 Cue 参数（live §11.5 判例原样：RR-1 Feeding-only + LightAvailability / Cue Profile——低光不另开通道）；TYPED 族参数差异（夜相底层气味/振动呈现（嗅觉灵敏方向 [需正文]）——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@AelBenthicBurrowNightHabitatProfile @AelLowLightSpatialProfile @AelPreyFields @AelDietClasses @AelSizeWindow @AelNormalFeedingProfile @AelLightCueProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| AEL_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| FactorType(typed) | habitat_factor：底层掩体/洞穴夜行底板轴 |
| SpatialSlotProfile | @AelLowLightSpatialProfile（低光空间槽——live §11.5 DynamicSpatialSlot 判例） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@AelPreyFields；diet_classes=@AelDietClasses；size_window=@AelSizeWindow） |
| LiveLayerProjection | BA-T1 底板 + DynamicSpatialSlot=@AelLowLightSpatialProfile（live §11.5 原样判例读法；两层 reconciliation OPEN——README §3） |
...
读取 当前格子的habitat_factor事实（底层掩体/洞穴夜行底板轴）
读取 当前格子的低光空间事实（低光/夜相空间槽——DynamicSpatialSlot）
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@AelPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@AelDietClasses 食性过滤
      与 size_window=@AelSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

第 1 步 夜行底板栖息档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中）：
    用habitat_factor事实查询 @AelBenthicBurrowNightHabitatProfile 的夜行底板分档槽
    （底层掩体/洞穴夜行底板轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 夜行底板档（preferred 槽）：
        BenthicBurrowNightHabitatFit = 全额
    否则如果 ∈ 次级夜栖档（tolerated 槽）：
        BenthicBurrowNightHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（无夜行底板档）：
        返回 0（EARLY_RETURN：无夜行底板的格子出局——夜行型栖息底板判据）

第 2 步 低光/夜相槽档位（APPLY_DYNAMIC_SPATIAL_SLOT 槽内三档——光照+时段判据；
  槽位置=live §11.5 判例固定原位，非作者可选）：
    用低光空间事实查询 @AelLowLightSpatialProfile 的低光分档槽
...
第 3 步 NORMALIZE_WEIGHT：
    对 BenthicBurrowNightHabitatFit（含低光槽合入调整）执行模板固定归一化（族常量，非作者可选）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @AelNormalFeedingProfile + @AelLightCueProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（夜相底层气味/振动呈现（嗅觉灵敏方向 [需正文]）——组样板参数）评价 @AelNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@AelNormalFeedingProfile 值域不冻结）：
...
- 放弃的自由度：(1) 归族裁决权移交（无 census 快照——Story 正文到达后判同可能改判，结构变更需重审）；(2) 合并算子（OPERATOR UNDEFINED）；(3) 数值与 Profile 值域不冻结。- 感官边界：鳗类嗅觉灵敏与 scent cue 轴（ScentCueProfile 资产）的关系未在本地 Story 证据——不并入该轴，仅 ctx 文案（同斑点叉尾鮰注记；先例引用不写 @ 前缀）。
