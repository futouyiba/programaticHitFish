# 彩虹镖鲈（Rainbow Darter｜Etheostoma caeruleum）｜伏击型四面生产级表达
...
| Story | FISH-R10 批成员（R10 摘要点名「镖鲈科首例」；CSV 唯一镖鲈科行＝彩虹镖鲈——首例具体种身份 [需核对]） |
| 冻结 Pattern | P01 [需核对]（R10 点名锚；行级标签未在本地） |
| 物种属性锚 | fish-reference-20260908：孤僻、晨昏活跃、肉食性、营养级 3.3（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- 摄食形态：彩虹镖鲈（底层砾石伏击）。
- 证据边界：Story 行级 Pattern 标签不在本地 [需核对]；本文件结构为伏击组样板（Tier A 样板骨架）的表达层选择——Story 正文到达后 census 判同可能改判（换组/换 BakeTemplate＝**结构变更需重审**，validator 族边界拦截静默改写），不是 Profile 重绑定。
- R10 点名注记：镖鲈科首例的 census 判同未做（B3+）——本文件为表达层骨架。
- Response 面：TYPED 族参数差异（底层无脊椎贴近呈现/停顿——组样板语义，参数级无新拓扑）。
- Group 面 / Quality 面：组样板 NO_SURFACE_EFFECT。
- 表达超集说明：骨架参数化表达；未超出组样板族域。


- **判断顺序（REP-ORDER-FIX-004 顺序还原，Tier B）**：判断链＝GATE_ZONE 门（底层水层定位——镖鲈科底栖特化，不成立=EARLY_RETURN） → 掩体结构档位（三档分级命中） → 归一化。水温/光照/时段等通用因子未入链（CSV 锚无 Story 空间程序证据——§5.1 例序是模板示例不是证据，不冒充）。

Profile 引用清单：@RbdGravelStructureProfile @RbdPreyFields @RbdDietClasses @RbdSizeWindow @RbdNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| RBD_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| FactorType(typed) | structure_factor：底层砾石结构轴（底砾/卵石滩贴近——底层小型伏击方向） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@RbdPreyFields；diet_classes=@RbdDietClasses；size_window=@RbdSizeWindow） |
...
判断链＝GATE_ZONE 门（底层水层定位——镖鲈科底栖特化） → 掩体结构档位 → 归一化；掩体档分级命中
...
读取 当前格子的structure_factor事实（底层砾石结构轴（底砾/卵石滩贴近——底层小型伏击方向））
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@RbdPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@RbdDietClasses 食性过滤
      与 size_window=@RbdSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

第 1 步 GATE_ZONE（底层水层定位——镖鲈科底栖特化——伏击型第一判断：无可用掩体不伏击）：
    用该 typed 因子轴事实查询 @RbdGravelStructureProfile 的掩体存在分档槽
    （档位成员=Profile 值域不冻结 [需正文]）
    如果 格子位于底层水层带（底砾/卵石滩水层）：
        进入第 2 步
    否则：
...
第 2 步 掩体结构档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中）：
    用该 typed 因子轴事实查询 @RbdGravelStructureProfile 的掩体伏击分档槽
    （底层砾石结构贴近质量：底砾滩/卵石间；档位成员=Profile 值域不冻结 [需正文]）
    如果 掩体结构 ∈ 最适伏击档（preferred 槽）：
        GravelAmbushFit = 全额
    否则如果 ∈ 次级掩体档（tolerated 槽）：
        GravelAmbushFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（暴露无掩体档）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；暴露格子不承载伏击分布）

第 3 步 NORMALIZE_WEIGHT：
    对 GravelAmbushFit 执行模板固定归一化（族常量，非作者可选）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @RbdNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（底层无脊椎贴近呈现/停顿——组样板参数）评价 @RbdNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@RbdNormalFeedingProfile 值域不冻结）：
...
- 使用的自由度：SINGLE 族投影标签与 typed 因子实例语义；Profile 命名；顺序还原链序与档位结构（REP-ORDER-FIX-004：门先行 EARLY_RETURN+掩体档三档分级命中）。
- 放弃的自由度：(1) 归族裁决权移交（无 census 快照——Story 正文到达后判同可能改判，结构变更需重审）；(2) 合并算子（无 combine 步；OPERATOR UNDEFINED）；(3) 数值与 Profile 值域不冻结。- [需核对] R10「镖鲈科首例」具体种身份（按 CSV 唯一镖鲈科行承载）。
