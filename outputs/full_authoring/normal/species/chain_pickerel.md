# 链纹狗鱼（Chain Pickerel｜Esox niger）｜追击型四面生产级表达
...
| 物种属性锚 | fish-reference-20260908：追猎、晨昏活跃、肉食性、营养级 3.29（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
Profile 引用清单：@CpkForageSchoolProfile @CpkVegetationCoverHabitatProfile @CpkPreyFields @CpkDietClasses @CpkSizeWindow @CpkNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| CPK_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| Factor2Type(typed) | habitat_factor：植被/掩体结构轴 |
| CombineRule | Template-fixed COMBINE_WEIGHTED（数学 OPERATOR UNDEFINED 待机制侧） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@CpkPreyFields；diet_classes=@CpkDietClasses；size_window=@CpkSizeWindow） |
...
读取 当前格子的habitat_factor事实（植被/掩体结构轴）
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@CpkPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@CpkDietClasses 食性过滤
      与 size_window=@CpkSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

槽 1 EVAL_TYPED_FIELD_OR_FACTOR（forage_factor，分级命中）：
    用forage_factor事实查询 @CpkForageSchoolProfile 的猎物场分档槽
...
槽 2 EVAL_TYPED_FIELD_OR_FACTOR（habitat_factor，分级命中）：
    用habitat_factor事实查询 @CpkVegetationCoverHabitatProfile 的栖息分档槽
    （植被/掩体结构轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 适配档（preferred 槽）：
        VegetationCoverHabitatFit = 全额
    否则如果 ∈ 过渡档（tolerated 槽）：
        VegetationCoverHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（排除档）：
        VegetationCoverHabitatFit = 出局槽值（excluded——非 EARLY_RETURN，同上族域边界）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @CpkNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（快速呈现/逃窜触发——组样板参数）评价 @CpkNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@CpkNormalFeedingProfile 值域不冻结）：
...
- 放弃的自由度：(1) 归族裁决权移交（无 census 快照——Story 正文到达后判同可能改判，结构变更需重审）；(2) 合并算子（OPERATOR UNDEFINED）；(3) 数值与 Profile 值域不冻结。- 同属互指：白斑狗鱼三 Story 分工在案——不继承。
