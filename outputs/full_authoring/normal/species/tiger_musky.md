...
Profile 引用清单：@TmkForageSchoolProfile @TmkOpenEdgeHabitatProfile @TmkPreyFields @TmkDietClasses @TmkSizeWindow @TmkNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| TMK_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| Factor1Type(typed) | forage_factor：猎物鱼群场轴（饵鱼群脉冲方向） |
| Factor2Type(typed) | habitat_factor：开放水/植被缘结构轴 |
| CombineRule | Template-fixed COMBINE_WEIGHTED（数学 OPERATOR UNDEFINED 待机制侧） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@TmkPreyFields；diet_classes=@TmkDietClasses；size_window=@TmkSizeWindow） |
...
读取 当前格子的forage_factor事实（猎物鱼群场轴（饵鱼群脉冲方向））
读取 当前格子的habitat_factor事实（开放水/植被缘结构轴）
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@TmkPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@TmkDietClasses 食性过滤
      与 size_window=@TmkSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

槽 1 EVAL_TYPED_FIELD_OR_FACTOR（forage_factor，分级命中）：
    用forage_factor事实查询 @TmkForageSchoolProfile 的猎物场分档槽
    （猎物鱼群场轴·饵鱼群脉冲方向；档位成员=Profile 值域不冻结 [需正文]）
...
槽 2 EVAL_TYPED_FIELD_OR_FACTOR（habitat_factor，分级命中）：
    用habitat_factor事实查询 @TmkOpenEdgeHabitatProfile 的栖息分档槽
    （开放水/植被缘结构轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 适配档（preferred 槽）：
        OpenEdgeHabitatFit = 全额
    否则如果 ∈ 过渡档（tolerated 槽）：
        OpenEdgeHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（排除档）：
        OpenEdgeHabitatFit = 出局槽值（excluded——非 EARLY_RETURN，同上族域边界）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @TmkNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（快速呈现/逃窜触发——组样板参数）评价 @TmkNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@TmkNormalFeedingProfile 值域不冻结）：
...
- 放弃的自由度：(1) 归族裁决权移交（无 census 快照——Story 正文到达后判同可能改判，结构变更需重审）；(2) 合并算子（OPERATOR UNDEFINED）；(3) 数值与 Profile 值域不冻结。- [需核对] R08「虎纹梭鱼 Identity-Deferred」与本 CSV 杂交行的对应关系（名实对齐后可能换 Profile 前缀或补文件）。同属互指：白斑狗鱼三 Story（本批 2 文件+migration 批 1 文件）/北美狗鱼/链纹狗鱼先例互指不继承。
