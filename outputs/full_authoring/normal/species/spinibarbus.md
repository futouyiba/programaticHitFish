# 光倒刺鲃（Spinibarbus｜Spinibarbus hollandi）｜伏击型四面生产级表达
...
- Response 面：TYPED 族参数差异（流区漂饵贴近呈现——组样板语义，参数级无新拓扑）。
- Group 面 / Quality 面：组样板 NO_SURFACE_EFFECT。
- 表达超集说明：骨架参数化表达；未超出组样板族域。


- **判断顺序（REP-ORDER-FIX-004 顺序还原，Tier B）**：判断链＝GATE_ROCK_CREVICE 门（急流石隙掩体存在，不成立=EARLY_RETURN） → 掩体结构档位（三档分级命中） → 归一化。水温/光照/时段等通用因子未入链（CSV 锚无 Story 空间程序证据——§5.1 例序是模板示例不是证据，不冒充）。

Profile 引用清单：@SbbRiffleRockStructureProfile @SbbPreyFields @SbbDietClasses @SbbSizeWindow @SbbNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| SBB_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| FactorType(typed) | structure_factor：急流石隙/掩体轴（石隙贴近方向） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@SbbPreyFields；diet_classes=@SbbDietClasses；size_window=@SbbSizeWindow） |
...
判断链＝GATE_ROCK_CREVICE 门（急流石隙掩体存在） → 掩体结构档位 → 归一化；掩体档分级命中
...
读取 当前格子的structure_factor事实（急流石隙/掩体轴（石隙贴近方向））
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@SbbPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@SbbDietClasses 食性过滤
      与 size_window=@SbbSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

第 1 步 GATE_ROCK_CREVICE（急流石隙掩体存在——伏击型第一判断：无可用掩体不伏击）：
    用该 typed 因子轴事实查询 @SbbRiffleRockStructureProfile 的掩体存在分档槽
...
第 2 步 掩体结构档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中）：
    ...
    （石隙贴近质量：石隙内/石块背流面；档位成员=Profile 值域不冻结 [需正文]）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @SbbNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（流区漂饵贴近呈现——组样板参数）评价 @SbbNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@SbbNormalFeedingProfile 值域不冻结）：
