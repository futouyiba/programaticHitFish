# 鳡鱼（Yellowcheek｜Elopichthys bambusa）｜机会组四面生产级表达
...
| 物种属性锚 | fish-reference-20260908：活泼、早晨活跃、食性列空 [需核对]（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- Response 面：TYPED 族参数差异（上层快速呈现/逃窜触发——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@YckSurfacePulsePatchProfile @YckPreyFields @YckDietClasses @YckSizeWindow @YckNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| YCK_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| FactorType(typed) | resource_patch：上层鱼群脉冲场轴（上层猎物丰度脉冲方向） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@YckPreyFields；diet_classes=@YckDietClasses；size_window=@YckSizeWindow） |
...
读取 当前格子的resource_patch事实（上层鱼群脉冲场轴（上层猎物丰度脉冲方向））
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@YckPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@YckDietClasses 食性过滤
      与 size_window=@YckSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

第 1 步 机会场食物丰度档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中——
  机会型第一判断：食物丰度先行）：
    用resource_patch事实查询 @YckSurfacePulsePatchProfile 的机会场丰度分档槽
    （上层鱼群脉冲场轴·上层猎物丰度脉冲方向；档位成员=Profile 值域不冻结 [需正文]）
    如果 机会场 ∈ 丰档（preferred 槽）：
        SurfacePulseFit = 全额
    否则如果 ∈ 贫档（tolerated 槽）：
        SurfacePulseFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（枯竭档——无可食机会）：
        返回 0（EARLY_RETURN：无食物机会的格子出局——机会型跟着食物走）

第 2 步 NORMALIZE_WEIGHT：
    对 SurfacePulseFit 执行模板固定归一化（族常量，非作者可选）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @YckNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（上层快速呈现/逃窜触发——组样板参数）评价 @YckNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@YckNormalFeedingProfile 值域不冻结）：
...
- 放弃的自由度：(1) 归族裁决权移交（无 census 快照——Story 正文到达后判同可能改判，结构变更需重审）；(2) 合并算子（无 combine 步；OPERATOR UNDEFINED）；(3) 数值与 Profile 值域不冻结。- [需核对] CSV 食性列空行的猎物构成（Profile 值域待正文）。
