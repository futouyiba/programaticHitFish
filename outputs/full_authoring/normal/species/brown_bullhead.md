# 云斑鮰（Brown Bullhead｜Ameiurus nebulosus）｜夜行低光四面生产级表达
...
| 物种属性锚 | fish-reference-20260908：躲藏、夜间活跃、肉食性、营养级 3.68（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- Response 面：R-T1 Feeding-only + 光照/低光 Cue 参数（live §11.5 判例原样：RR-1 Feeding-only + LightAvailability / Cue Profile——低光不另开通道）；TYPED 族参数差异（夜相底层气味/振动呈现——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@BbhMudSlackNightHabitatProfile @BbhLowLightSpatialProfile @BbhPreyFields @BbhDietClasses @BbhSizeWindow @BbhNormalFeedingProfile @BbhLightCueProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
| FactorType(typed) | habitat_factor：泥底/缓流夜行底板轴 |
...
读取 当前格子的habitat_factor事实（泥底/缓流夜行底板轴）
...
第 1 步 夜行底板栖息档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中）：
    用habitat_factor事实查询 @BbhMudSlackNightHabitatProfile 的夜行底板分档槽
    （泥底/缓流夜行底板轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 夜行底板档（preferred 槽）：
        MudSlackNightHabitatFit = 全额
    否则如果 ∈ 次级夜栖档（tolerated 槽）：
        MudSlackNightHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（无夜行底板档）：
...
第 3 步 NORMALIZE_WEIGHT：
    对 MudSlackNightHabitatFit（含低光槽合入调整）执行模板固定归一化（族常量，非作者可选）
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（夜相底层气味/振动呈现——组样板参数）评价 @BbhNormalFeedingProfile
    得到 FoodEvaluation
