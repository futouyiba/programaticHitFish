...
第 1 步 机会场食物丰度档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中——
  机会型第一判断：食物丰度先行）：
    用resource_patch事实查询 @StbBaitPulsePatchProfile 的机会场丰度分档槽
    （饵鱼群脉冲场轴·洄游饵鱼群方向；档位成员=Profile 值域不冻结 [需正文]）
    如果 机会场 ∈ 丰档（preferred 槽）：
        BaitPulseFit = 全额
    否则如果 ∈ 贫档（tolerated 槽）：
        BaitPulseFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（枯竭档——无可食机会）：
...
Feeding Channel（摄食通道）：
    EVAL_TARGET_AS_FOOD_TYPED（分级命中，REP-ORDER-FIX-004 展开）：
        按三档判定（档位成员=@StbNormalFeedingProfile 值域不冻结 [需正文]）：
