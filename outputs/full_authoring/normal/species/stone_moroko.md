# 麦穗鱼（Stone Moroko｜Pseudorasbora parva）｜机会组四面生产级表达
...
| 物种属性锚 | fish-reference-20260908：撕鳍、全天活跃、杂食性、营养级 3.06（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- Response 面：TYPED 族参数差异（小饵/漂饵机会呈现（撕鳍攻击性并入参数 [需正文]）——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@StmDriftInvertPatchProfile @StmPreyFields @StmDietClasses @StmSizeWindow @StmNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
| FactorType(typed) | resource_patch：缓流无脊椎/碎屑机会场轴（小型机会摄食方向） |
...
读取 当前格子的resource_patch事实（缓流无脊椎/碎屑机会场轴（小型机会摄食方向））
读取 当前格子的可食资源原始事实
...
第 1 步 机会场食物丰度档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中——
  机会型第一判断：食物丰度先行）：
    用resource_patch事实查询 @StmDriftInvertPatchProfile 的机会场丰度分档槽
    （缓流无脊椎/碎屑机会场轴·小型机会摄食方向；档位成员=Profile 值域不冻结 [需正文]）
    如果 机会场 ∈ 丰档（preferred 槽）：
        DriftInvertFit = 全额
    否则如果 ∈ 贫档（tolerated 槽）：
        DriftInvertFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（枯竭档——无可食机会）：
        返回 0（EARLY_RETURN：无食物机会的格子出局——机会型跟着食物走）

第 2 步 NORMALIZE_WEIGHT：
    对 DriftInvertFit 执行模板固定归一化（族常量，非作者可选）
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（小饵/漂饵机会呈现（撕鳍攻击性并入参数 [需正文]）——组样板参数）评价 @StmNormalFeedingProfile
    得到 FoodEvaluation
...
- 放弃的自由度：(1) 归族裁决权移交（无 census 快照——Story 正文到达后判同可能改判，结构变更需重审）；(2) 合并算子（无 combine 步；OPERATOR UNDEFINED）；(3) 数值与 Profile 值域不冻结。- CSV 性格=撕鳍的攻击性语义归 Response 参数；若 Story 判 Reaction 通道主导＝结构变更需重审。
