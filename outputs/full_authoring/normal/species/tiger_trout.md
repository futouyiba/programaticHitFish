# 虎纹鳟（Tiger Trout｜Salmo trutta × Salvelinus fontinalis）｜追击型四面生产级表达
...
| 物种属性锚 | fish-reference-20260908：追猎、晨昏活跃、肉食性、营养级 4.14（行级 AI 审核状态=待人工审核；仅作方向锚；杂交行） |
...
Profile 引用清单：@TtrForageSchoolProfile @TtrDeepStructureHabitatProfile @TtrPreyFields @TtrDietClasses @TtrSizeWindow @TtrNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| TTR_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| Factor2Type(typed) | habitat_factor：深潭/结构轴 |
| CombineRule | Template-fixed COMBINE_WEIGHTED（数学 OPERATOR UNDEFINED 待机制侧） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@TtrPreyFields；diet_classes=@TtrDietClasses；size_window=@TtrSizeWindow） |
...
读取 当前格子的habitat_factor事实（深潭/结构轴）
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@TtrPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@TtrDietClasses 食性过滤
      与 size_window=@TtrSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

槽 1 EVAL_TYPED_FIELD_OR_FACTOR（forage_factor，分级命中）：
    用forage_factor事实查询 @TtrForageSchoolProfile 的猎物场分档槽
...
槽 2 EVAL_TYPED_FIELD_OR_FACTOR（habitat_factor，分级命中）：
    用habitat_factor事实查询 @TtrDeepStructureHabitatProfile 的栖息分档槽
    （深潭/结构轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 适配档（preferred 槽）：
        DeepStructureHabitatFit = 全额
    否则如果 ∈ 过渡档（tolerated 槽）：
        DeepStructureHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（排除档）：
        DeepStructureHabitatFit = 出局槽值（excluded——非 EARLY_RETURN，同上族域边界）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @TtrNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（快速呈现/逃窜触发——组样板参数）评价 @TtrNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@TtrNormalFeedingProfile 值域不冻结）：
...
- 放弃的自由度：(1) 归族裁决权移交（无 census 快照——Story 正文到达后判同可能改判，结构变更需重审）；(2) 合并算子（OPERATOR UNDEFINED）；(3) 数值与 Profile 值域不冻结。- [需核对] 杂交行身份（褐鳟×美洲红点鲑杂交种——亲本双文件在本批/migration 批在案，杂交行独立承载）。
