# 蒙古红鲌（Mongolian Redfin｜Chanodichthys mongolicus）｜伏击型四面生产级表达
...
Profile 引用清单：@MgfEdgeShadeStructureProfile @MgfPreyFields @MgfDietClasses @MgfSizeWindow @MgfNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| MGF_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| Bake 输入契约 | UsableForageAvailability（prey_fields=@MgfPreyFields；diet_classes=@MgfDietClasses；size_window=@MgfSizeWindow） |
...
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@MgfPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@MgfDietClasses 食性过滤
      与 size_window=@MgfSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

第 1 步 GATE_SHADE_EDGE（明暗交界带定位——中上层伏击位——伏击型第一判断：无可用掩体不伏击）：
    用该 typed 因子轴事实查询 @MgfEdgeShadeStructureProfile 的掩体存在分档槽
...
第 2 步 掩体结构档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中）：
    用该 typed 因子轴事实查询 @MgfEdgeShadeStructureProfile 的掩体伏击分档槽
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @MgfNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（明暗交界带呈现/清晨窗口——组样板参数）评价 @MgfNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@MgfNormalFeedingProfile 值域不冻结）：
...
- 放弃的自由度：(1) 归族裁决权移交（无 census 快照——Story 正文到达后判同可能改判，结构变更需重审）；(2) 合并算子（无 combine 步；OPERATOR UNDEFINED）；(3) 数值与 Profile 值域不冻结。- 组间张力登记：同翘嘴红鲌（README §4 登记项 5）。
