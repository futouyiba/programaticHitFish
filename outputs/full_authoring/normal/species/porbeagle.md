# 鼠鲨（Porbeagle｜Lamna nasus）｜追击型四面生产级表达
...
| 物种属性锚 | fish-reference-20260908：追猎、全天活跃、肉食性、营养级 4.57（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- Response 面：TYPED 族参数差异（中速持续呈现/游动诱饵——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@PblSchoolingForageProfile @PblOpenThermoclineHabitatProfile @PblPreyFields @PblDietClasses @PblSizeWindow @PblNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| PBL_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| 字段 | 值 |
|---|---|
| BakeTemplate | BA-P01-PURSUIT-PLAIN（本批投影标签＝census PLAIN_FACTOR_COMBINE，registry v4；因子集→组合——追击型双因子投影；**§2.2 已顺序还原（REP-ORDER-FIX-004）：槽内三档分级命中——槽间顺序按族契约 unordered 不还原，登记 README §7**） |
| Factor1Type(typed) | forage_factor：鱼群猎物场轴（鲱形类群游方向） |
| Factor2Type(typed) | habitat_factor：开放水/温跃层轴 |
| CombineRule | Template-fixed COMBINE_WEIGHTED（数学 OPERATOR UNDEFINED 待机制侧） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@PblPreyFields；diet_classes=@PblDietClasses；size_window=@PblSizeWindow） |
...
读取 当前格子的forage_factor事实（鱼群猎物场轴（鲱形类群游方向））
读取 当前格子的habitat_factor事实（开放水/温跃层轴）
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@PblPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@PblDietClasses 食性过滤
      与 size_window=@PblSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

槽 1 EVAL_TYPED_FIELD_OR_FACTOR（forage_factor，分级命中）：
    用forage_factor事实查询 @PblSchoolingForageProfile 的猎物场分档槽
    （鱼群猎物场轴·鲱形类群游方向；档位成员=Profile 值域不冻结 [需正文]）
    如果 猎物场 ∈ 丰档（preferred 槽）：
        SchoolingForageFit = 全额
    否则如果 ∈ 贫档（tolerated 槽）：
        SchoolingForageFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（无猎物档）：
        SchoolingForageFit = 出局槽值（excluded——非 EARLY_RETURN，出槽值仍进 COMBINE；
        PLAIN 族域内槽无 gate 语义）

槽 2 EVAL_TYPED_FIELD_OR_FACTOR（habitat_factor，分级命中）：
    用habitat_factor事实查询 @PblOpenThermoclineHabitatProfile 的栖息分档槽
    （开放水/温跃层轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 适配档（preferred 槽）：
        OpenThermoclineHabitatFit = 全额
    否则如果 ∈ 过渡档（tolerated 槽）：
        OpenThermoclineHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（排除档）：
        OpenThermoclineHabitatFit = 出局槽值（excluded——非 EARLY_RETURN，同上族域边界）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @PblNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（中速持续呈现/游动诱饵——组样板参数）评价 @PblNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@PblNormalFeedingProfile 值域不冻结）：
...
- 使用的自由度：族投影标签与 typed 因子实例语义；Profile 命名；顺序还原链序与档位结构（REP-ORDER-FIX-004：槽内三档分级命中——槽间顺序按族契约 unordered 不还原）。
- 放弃的自由度：(1) 归族裁决权移交（无 census 快照——Story 正文到达后判同可能改判，结构变更需重审）；(2) 合并算子（OPERATOR UNDEFINED）；(3) 数值与 Profile 值域不冻结。- 同纲互指：白斑角鲨（本批 SENSE 组）被动电感知例在案——鼠鲨行无感官点名，不并入 cue 轴（无 Story 证据不冒充）。
