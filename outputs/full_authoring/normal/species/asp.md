# 赤稍雅罗鱼（Asp｜Leuciscus aspius）｜追击型四面生产级表达
...
| 物种属性锚 | fish-reference-20260908：追猎、早晨活跃、肉食性、营养级 4.47（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
Profile 引用清单：@AspSurfaceSchoolProfile @AspOpenSurfaceHabitatProfile @AspPreyFields @AspDietClasses @AspSizeWindow @AspNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| ASP_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| Factor1Type(typed) | forage_factor：上层小鱼群场轴 |
| Factor2Type(typed) | habitat_factor：开放水面/急流顶轴 |
| CombineRule | Template-fixed COMBINE_WEIGHTED（数学 OPERATOR UNDEFINED 待机制侧） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@AspPreyFields；diet_classes=@AspDietClasses；size_window=@AspSizeWindow） |
...
读取 当前格子的forage_factor事实（上层小鱼群场轴）
读取 当前格子的habitat_factor事实（开放水面/急流顶轴）
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@AspPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@AspDietClasses 食性过滤
      与 size_window=@AspSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

槽 1 EVAL_TYPED_FIELD_OR_FACTOR（forage_factor，分级命中）：
    用forage_factor事实查询 @AspSurfaceSchoolProfile 的猎物场分档槽
    （上层小鱼群场轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 猎物场 ∈ 丰档（preferred 槽）：
        SurfaceSchoolFit = 全额
    否则如果 ∈ 贫档（tolerated 槽）：
        SurfaceSchoolFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（无猎物档）：
        SurfaceSchoolFit = 出局槽值（excluded——非 EARLY_RETURN，出槽值仍进 COMBINE；
        PLAIN 族域内槽无 gate 语义）

槽 2 EVAL_TYPED_FIELD_OR_FACTOR（habitat_factor，分级命中）：
    用habitat_factor事实查询 @AspOpenSurfaceHabitatProfile 的栖息分档槽
    （开放水面/急流顶轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 适配档（preferred 槽）：
        OpenSurfaceHabitatFit = 全额
    否则如果 ∈ 过渡档（tolerated 槽）：
        OpenSurfaceHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（排除档）：
        OpenSurfaceHabitatFit = 出局槽值（excluded——非 EARLY_RETURN，同上族域边界）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @AspNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（水面快速呈现/逃窜触发——组样板参数）评价 @AspNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@AspNormalFeedingProfile 值域不冻结）：
