# 斑点叉尾鮰（Channel Catfish｜Ictalurus punctatus）｜夜行低光四面生产级表达
...
| 物种属性锚 | fish-reference-20260908：孤僻、夜间活跃、杂食性、营养级 4.16（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- Response 面：R-T1 Feeding-only + 光照/低光 Cue 参数（live §11.5 判例原样：RR-1 Feeding-only + LightAvailability / Cue Profile——低光不另开通道）；TYPED 族参数差异（夜相底层气味/振动呈现（杂食机会——触须味觉方向 [需正文]）——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@CcfGravelCoverNightHabitatProfile @CcfLowLightSpatialProfile @CcfPreyFields @CcfDietClasses @CcfSizeWindow @CcfNormalFeedingProfile @CcfLightCueProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| CCF_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| 字段 | 值 |
|---|---|
| BakeTemplate | BA-P01-NOCTURNAL-SINGLE（本批投影标签＝census SINGLE_FACTOR_NORMALIZED_WEIGHT 低光侧；live §11.5 判例：MERGE_SUPPORTED → BA-T1 + 低光空间槽，新增 BakeTemplate = +0；**§2.2 已顺序还原（REP-ORDER-FIX-004）：底板档三档+低光槽内三档（槽位置按 §11.5 判例原位），登记 README §7**） |
| FactorType(typed) | habitat_factor：河底砂砾/掩体夜行底板轴 |
| SpatialSlotProfile | @CcfLowLightSpatialProfile（低光空间槽——live §11.5 DynamicSpatialSlot 判例） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@CcfPreyFields；diet_classes=@CcfDietClasses；size_window=@CcfSizeWindow） |
| LiveLayerProjection | BA-T1 底板 + DynamicSpatialSlot=@CcfLowLightSpatialProfile（live §11.5 原样判例读法；两层 reconciliation OPEN——README §3） |
...
读取 当前格子的habitat_factor事实（河底砂砾/掩体夜行底板轴）
读取 当前格子的低光空间事实（低光/夜相空间槽——DynamicSpatialSlot）
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@CcfPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@CcfDietClasses 食性过滤
      与 size_window=@CcfSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

第 1 步 夜行底板栖息档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中）：
    用habitat_factor事实查询 @CcfGravelCoverNightHabitatProfile 的夜行底板分档槽
    （河底砂砾/掩体夜行底板轴；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 夜行底板档（preferred 槽）：
        GravelCoverNightHabitatFit = 全额
    否则如果 ∈ 次级夜栖档（tolerated 槽）：
        GravelCoverNightHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（无夜行底板档）：
        返回 0（EARLY_RETURN：无夜行底板的格子出局——夜行型栖息底板判据）

第 2 步 低光/夜相槽档位（APPLY_DYNAMIC_SPATIAL_SLOT 槽内三档——光照+时段判据；
  槽位置=live §11.5 判例固定原位，非作者可选）：
    用低光空间事实查询 @CcfLowLightSpatialProfile 的低光分档槽
...
第 3 步 NORMALIZE_WEIGHT：
    对 GravelCoverNightHabitatFit（含低光槽合入调整）执行模板固定归一化（族常量，非作者可选）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @CcfNormalFeedingProfile + @CcfLightCueProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（夜相底层气味/振动呈现（杂食机会——触须味觉方向 [需正文]）——组样板参数）评价 @CcfNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@CcfNormalFeedingProfile 值域不冻结）：
...
- 使用的自由度：族投影标签与 typed 因子实例语义；Profile 命名；顺序还原链序与档位结构（REP-ORDER-FIX-004：夜行底板档三档+低光槽内三档——槽位置按 §11.5 判例原位）。
- 放弃的自由度：(1) 归族裁决权移交（无 census 快照——Story 正文到达后判同可能改判，结构变更需重审）；(2) 合并算子（OPERATOR UNDEFINED）；(3) 数值与 Profile 值域不冻结。- 杂食性归层注记：机会组语义并入夜行组主结构（夜行时段锚优先）——嗅觉 cue 轴（ScentCueProfile 资产，先例引用不写 @ 前缀）未并入：本地无该鱼的 scent Story 证据（README §4 登记项 5）。
