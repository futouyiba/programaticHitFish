# 唇䱻（Labeo Barbel｜Hemibarbus labeo）｜机会组四面生产级表达
...
| 物种属性锚 | fish-reference-20260908：活泼、全天活跃、食性列空 [需核对]（行级 AI 审核状态=待人工审核；仅作方向锚） |
...
- Response 面：TYPED 族参数差异（底层缓动/贴底呈现——组样板语义，参数级无新拓扑）。
...
Profile 引用清单：@LbbBenthicInvertPatchProfile @LbbPreyFields @LbbDietClasses @LbbSizeWindow @LbbNormalFeedingProfile @NeutralEligibility @NeutralAffinity @SpeciesBaseQualityProfile
...
|---|---|---|---|---|
| LBB_Default | 默认 | NormalFeeding | 承接剩余份额 | 1 - SpecialShareTotal |
...
| FactorType(typed) | resource_patch：底栖无脊椎机会场轴（触须底觅方向） |
| Normalization | NORMALIZE_WEIGHT（族常量；非作者可选算子） |
| Bake 输入契约 | UsableForageAvailability（prey_fields=@LbbPreyFields；diet_classes=@LbbDietClasses；size_window=@LbbSizeWindow） |
...
读取 当前格子的resource_patch事实（底栖无脊椎机会场轴（触须底觅方向））
读取 当前格子的可食资源原始事实
    （UsableForageAvailability 契约输出：
      prey_fields=@LbbPreyFields 绑定的 prey class 生物量，
      经 diet_classes=@LbbDietClasses 食性过滤
      与 size_window=@LbbSizeWindow 口径过滤——在场可食生物量，未经感知/捕获修正）

第 1 步 机会场食物丰度档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中——
  机会型第一判断：食物丰度先行）：
    用resource_patch事实查询 @LbbBenthicInvertPatchProfile 的机会场丰度分档槽
    （底栖无脊椎机会场轴·触须底觅方向；档位成员=Profile 值域不冻结 [需正文]）
    如果 机会场 ∈ 丰档（preferred 槽）：
        BenthicInvertFit = 全额
    否则如果 ∈ 贫档（tolerated 槽）：
        BenthicInvertFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（枯竭档——无可食机会）：
        返回 0（EARLY_RETURN：无食物机会的格子出局——机会型跟着食物走）

第 2 步 NORMALIZE_WEIGHT：
    对 BenthicInvertFit 执行模板固定归一化（族常量，非作者可选）
...
|---|---|---|---|---|
| NormalFeeding | R-T1 单通道（Feeding） | @LbbNormalFeedingProfile | 返回 FeedingResponse | 返回低 / 无响应 |
...
EVAL_TARGET_AS_FOOD_TYPED：
    用目标事实（底层缓动/贴底呈现——组样板参数）评价 @LbbNormalFeedingProfile
    得到 FoodEvaluation

DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@LbbNormalFeedingProfile 值域不冻结）：
