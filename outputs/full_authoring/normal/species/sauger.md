...
第 1 步 夜行底板栖息档位（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中）：
    用habitat_factor事实查询 @SgrTurbidDeepNightHabitatProfile 的夜行底板分档槽
    （浑水深水/结构夜行底板轴·浑水适应视觉 [需正文]；档位成员=Profile 值域不冻结 [需正文]）
    如果 栖息 ∈ 夜行底板档（preferred 槽）：
        TurbidDeepNightHabitatFit = 全额
    否则如果 ∈ 次级夜栖档（tolerated 槽）：
        TurbidDeepNightHabitatFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（无夜行底板档）：
        返回 0（EARLY_RETURN：无夜行底板的格子出局——夜行型栖息底板判据）

第 2 步 低光/夜相槽档位（APPLY_DYNAMIC_SPATIAL_SLOT 槽内三档——光照+时段判据；
  槽位置=live §11.5 判例固定原位，非作者可选）：
    用低光空间事实查询 @SgrLowLightSpatialProfile 的低光分档槽
...
第 3 步 NORMALIZE_WEIGHT：
    对 TurbidDeepNightHabitatFit（含低光槽合入调整）执行模板固定归一化（族常量，非作者可选）
...
DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-004 展开）：
    按三档判定 FoodEvaluation（档位成员=@SgrNormalFeedingProfile 值域不冻结）：
