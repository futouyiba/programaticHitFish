...
- **判断顺序（REP-ORDER-FIX-003 顺序还原，样板语义层推导）**：判断链＝premise 读取（OCEAN/MIGRATION/SPAWN 配置级）→ 阶段绑定轴段归属三档 → 归一化。推导来源＝G2/C12 样板空间重排语义（海洋觅食肥育→溯河→淡水产卵三段轴；样板双层在案但样板未含步序——链序为样板语义的方向级还原，段成员 [需正文]）。分级命中：轴段三档（当前阶段偏好轴段=全额/河口等过渡带=削减不清零/轴段外=出局 EARLY_RETURN）；early return 的对象=格子（不在当前阶段绑定轴段的格子出局），不是阶段——阶段切换永远 premise 配置级（PREMBIND 不变量维持），非洄游阶段不出局鱼（程序按海洋段轴运行）；「非洄游阶段不进入洄游 Response 程序」由 §1 路由承载（阶段∉枚举→不生成 MigrationReaction Group，程序级出局），不进 Bake body 阶段分支。CSV benthopelagic 锚未入链（样板链优先）。档位成员与阈值全 Profile 值域不冻结。
...
| 字段 | 值 |
|---|---|
| BakeTemplate | BA-MIGRATION-SINGLE（本批投影标签＝census SINGLE_FACTOR_NORMALIZED_WEIGHT，registry v4；2 步单 typed 因子→归一化；照 census B2 北极红点鲑位置因子先例投影；**§2.2 已顺序还原（REP-ORDER-FIX-003）：轴段归属三档+early return 链，分歧登记 README §7**） |
...
不进 body 分支（PREMBIND 不变量维持：阶段=premise 配置级切换因子集，CHB/MGC 先例）——
early return 的对象是格子（不在当前阶段绑定轴段内的格子出局），不是阶段（非洄游阶段
程序仍运行、按海洋段轴取值）。推导来源=G2/C12 样板空间重排语义（方向级，段成员 [需正文]）；
...
第 1 步 阶段绑定轴段归属（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中）：
    用位置轴事实查询 @AtlanticSalmonMigrationSpatialProfile 的阶段轴段分档槽
...
DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-003 展开）：
    按三档判定 FoodEvaluation（档位成员=@AtlanticSalmonNormalFeedingProfile 值域不冻结）：
...
第 0 步 程序级门（非洄游阶段不进入本 Program——§1 路由承载）：
    阶段事实 ∉ @AtlanticSalmonMigrationStages 或 水体类型 ∉ @AtlanticSalmonMigrationWaterTypes
...
第 1 步 状态门控（STATE_GATE_FEEDING，停食判例——判断链最先）：
    本 Group 停食语义成立——普通 Feeding 强抑制或关闭
    （Typed Result=档位关闭而非数值归零——G2 后续 Response 原文
    ...
    最先关闭，后续步骤不再评价任何摄食响应；停食触点窗口=Profile 值域内定值，
    大西洋鲑停食实证口径 [需正文]——本文件按 C12 Case 语义表达）
...
第 3 步 DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-003 展开）：
    按三档判定 ProvocationEvaluation（档位成员=@AtlanticSalmonMigrationReactionProfile 值域不冻结）：
...
- 使用的自由度：G2 MigrationReaction share-vector 路由样板（条件原子 V2+R2 原样）；SINGLE 族投影标签与 typed 因子实例语义；Profile 命名；**顺序还原链序与档位结构（REP-ORDER-FIX-003：Bake 轴段归属三档+early return 链、Response 状态门控显式化+程序级门+档位展开——推导依据 §0 判断顺序行）**；C12 双 Path Response 结构。
- 放弃的自由度：(1) census canonical 步序的服从（顺序还原后链与 canonical 两步「无 gate 判语」拓扑分歧——登记 README §7，裁决归 census 侧族重跑）；(2) R-T2 折叠候选（两 Group 各 R-T1 vs 单程序固定 Feeding/Reaction 双 Channel——live §13.3 自身未闭合，折叠裁决归机制侧）；(3) BA-T7 ROUTE/TRANSITION 洄游专用句型（§11.6 判 NEW_TEMPLATE_NOT_PROVEN，corridor/staging path 顺序未证实须进入 SpatialDistributionWeight）；(4) 阶段选择器（G2 明确不购买；程序级门/状态门控不构成 body 内阶段分支）；(5) 合并算子（SINGLE 链无 combine 步；OPERATOR UNDEFINED）；(6) 数值与 Profile 值域不冻结（含轴段/档位成员）。
...
顺序还原修复批次：REP-ORDER-FIX-003（§0/§2/§3/§5 修改；Bake 轴段归属三档+early return 链，Response 状态门控显式化+程序级门+双 Path 档位展开）
