...
EVAL_TYPED_FIELD_OR_FACTOR：
    用位置轴事实查询 @WhiteSturgeonMigrationSpatialProfile
...
普通 Feeding 强抑制或关闭（停食判例族第 4 例证据：R09 摘要
「停食第 4 例（鲟科）」——停食语义首次进入鲟形目；
...
- 放弃的自由度：(1) R-T2 折叠候选（归机制侧）；(2) BA-T7 ROUTE/TRANSITION 洄游句型（NEW_TEMPLATE_NOT_PROVEN）；(3) 白化变体行分裂（色型不建体判例，资产维度归呈现层）；(4) 阶段选择器；(5) 合并算子（SINGLE 链无 combine 步；OPERATOR UNDEFINED）；(6) 数值与 Profile 值域不冻结。
