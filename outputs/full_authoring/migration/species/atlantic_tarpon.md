...
- **判断顺序（REP-ORDER-FIX-003 顺序还原，CSV 方向级推导 [需正文]）**：判断链＝premise 读取（发育阶段配置级——amphidromous 发育枚举，非繁殖阶段）→ 发育阶段绑定轴段归属三档 → 归一化。推导来源＝G2/C12 样板空间重排语义+CSV amphidromous 方向锚（Tier B 无行级证据，链序为样板语义方向级还原，段成员 [需正文]）。分级命中：轴段三档（当前发育阶段偏好轴段=全额——沿岸礁带↔河口↔河沼幼体带随 premise 取段/过渡带=削减不清零/轴段外=出局 EARLY_RETURN）；early return 的对象=格子，不是发育阶段（PREMBIND 不变量维持；发育阶段切换=premise 配置级，本 body 只取 Profile 轴段值）。CSV reef-associated 锚未入链（样板链优先）。档位成员与阈值全 Profile 值域不冻结 [需正文]。
...
判断链＝premise 读取（发育阶段配置级——amphidromous 发育枚举落 Profile 值域，非繁殖
阶段枚举）→ 发育阶段绑定轴段归属三档 → 归一化；轴段判定分级命中（当前阶段偏好
轴段=全额/过渡带=削减不清零/轴段外=出局 EARLY_RETURN）。发育阶段切换本身不进 body
分支（PREMBIND 不变量维持）；early return 的对象是格子，不是发育阶段（非洄游阶段
程序仍运行、按成体沿岸礁带轴取值）。推导来源=G2/C12 样板空间重排语义+CSV
amphidromous 方向锚（方向级，段成员 [需正文]）；档位成员=Profile 值域不冻结。
...
读取 当前 premise（发育阶段——上游 lifecycle trait，配置级切换因子集；
    LARVA/JUVENILE_RIVER/SUBADULT_SEA/ADULT_COASTAL 方向枚举落 Profile 值域；
    本 body 只按 premise 取 Profile 轴段值，不含发育阶段分支）

第 1 步 发育阶段绑定轴段归属（EVAL_TYPED_FIELD_OR_FACTOR 的顺序还原形，分级命中）：
    用位置轴事实查询 @TarponAmphidromousSpatialProfile 的阶段轴段分档槽
    （轴=沿岸礁带↔河口↔河沼幼体带；段成员与阈值=Profile 值域不冻结 [需正文]）
    如果 格子位置 ∈ 当前 premise 偏好轴段（preferred 槽）：
        AmphidromousSpatialFit = 全额
    否则如果 ∈ 过渡带（tolerated 槽——河口混交带方向）：
        AmphidromousSpatialFit = 削减（× Profile 衰减参数——削减但不清零）
    否则（当前发育阶段绑定轴段外——非本阶段空间）：
        返回 0.01 × weight（EARLY_RETURN ×0.01 软出局：非零、仍可参与下游——语义裁定 1/2[REP-WORDING-ALIGN-001]；格子不在当前发育阶段空间范围，出局）
...
DECIDE_RESPONSE（分级命中，REP-ORDER-FIX-003 展开）：
    按三档判定 FoodEvaluation（档位成员=@TarponNormalFeedingProfile 值域不冻结）：
...
- 使用的自由度：SINGLE 族投影标签；typed 因子实例语义（发育阶段位置轴——amphidromous 状态轴）；Profile 命名；**顺序还原链序与档位结构（REP-ORDER-FIX-003：轴段归属三档+early return 链、Response 档位展开——推导依据 §0 判断顺序行）**。
- 放弃的自由度：(1) census canonical 步序的服从（顺序还原后链与 canonical 两步「无 gate 判语」拓扑分歧——登记 README §7，裁决归 census 侧族重跑）；(2) 归族裁决权（结构变更需重审）；(3) MigrationReaction 双 Path（非繁殖洄游无停食证据——若正文证实阶段性停食按判例族另行评估）；(4) BA-T7 ROUTE/TRANSITION 句型；(5) 合并算子（OPERATOR UNDEFINED）；(6) 数值与 Profile 值域不冻结（含轴段/档位成员）。
