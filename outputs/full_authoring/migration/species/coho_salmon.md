...
EVAL_TYPED_FIELD_OR_FACTOR：
    用位置轴事实查询 @CohoSalmonMigrationSpatialProfile
...
- 放弃的自由度：(1) 归族裁决权（结构变更需重审）；(2) MigrationReaction 双 Path（停食未证实）；(3) BA-T7 ROUTE/TRANSITION 句型；(4) 合并算子（OPERATOR UNDEFINED）；(5) 数值与 Profile 值域不冻结。
