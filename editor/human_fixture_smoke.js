...
(async () => {
  ...
  // T1 Species Shared edit -> generated definition control updates inherited value; ProgramRef unchanged.
  ...
  const speciesWorkspace = await page.locator('#workspace').innerText();
  assert(speciesWorkspace.includes('Generated from ComponentDefinition owner, type and constraints'), 'T1 inspector is not visibly definition-driven');
  ...
  // FishQuality narrow override control must also be generated from ComponentDefinition.
  await page.getByRole('button', {name: 'FishQuality', exact: true}).click();
  assert(await page.getByLabel('prey_size_preference').count() === 1, 'FishQuality definition-driven control missing');
  ...
  await page.getByRole('button', {name: 'Engagement Mode Routing'}).click();
  assert((await page.locator('#workspace').innerText()).includes('Human fixture slice: explicit allocation + diagnostics'), 'Routing fixture scope boundary missing');
  ...
  console.log('HUMAN_FIXTURE_SMOKE_PASS: DEFINITION_DRIVEN T1 T2 T3 T4 T5 RESET');
  await browser.close();
})().catch(async error => {
  console.error(error);
  process.exit(1);
