...
- **产物**：template_registry.yaml v9（40 处锚点断言替换[REV-001 F4：原记 36 与脚本 40 rep() 不符]，脚本 apply_hrq_mutation.py 幂等留档）+ truth_rebuild_queue.jsonl（248 行）+ 本批档

## 1. 逐条裁决→落表对照

| 裁决 | 决策日志裁定 | registry 落表 | 状态 |
|---|---|---|---|
| 1 SINGLE 族 | A：拆分+234 AMB 转真形重验队列 | SINGLE status→RETIRED+retired_provenance（v1 证伪注记：RS1 61/61+渐进累积语义+饱和伪影根因）；61 moved＝47 转正 9 链族+14 slot_tiering 转 queue（原 66 条清单保留为历史快照层；REV-001 F2 补正）；5 无文件成员 evidence_insufficient_held（LAM/PIN/ASR/RVS/RDS 归 FR 线）；234→queue | 落表 |
...
| GUARD 名义 | 8（7+HNC 挂账） | 28 | +B5 9+B6 2+B7 9 全部转正；四形式正式 24[nest 12/egg_mass 7/fry_school 3/host_brood 2]+brooded 挂起 2[ARO/TIL3]+form-hold 挂起 2[CSL/CSN1（REV-001 F1）] |
...
| SINGLE | 66 名义（过渡双列） | 5 held（RETIRED） | 47 转正链族+14 转 queue（F2 补正）；LAM/PIN/ASR/RVS/RDS evidence_insufficient_held |
...
1. **GUARD 落位表未点名 8 成员**（CSL/ROB/CRA2/SMA1/BLU2/CCF2/CSN1/BBR1）：按裁决 4 四形式定义直接分类（巢构建型→nest；洞巢利用型→egg_mass+suitability；跨阶段成员四形式证据不足者挂 form_hold 不虚构——CSL/CSN1 两例经 REV-001 F1 改挂起）——registry four_form_disposition.ruling 已标注「独立审复核点」。
...
4. **GUARD participant 值**：仅对有文本证据的成员填（15 例：story 标题/提案文本直证 male 或 biparental——其中 DIS2 biparental 系 B1-DIS canonical 色型并入继承、非本 story 直证[REV-001 F3 补正]），其余不虚构、留重跑补全。
...
- 脚本守卫：40 处替换全部锚点唯一断言通过（原记 36 系转述笔误——脚本自报 patch count: 40；REV-001 F4）（1 处计数错误被守卫拦截后修正：review_queue HRQ-RS1-01/02 实为 7 处非 6——FILTER_FIELD 漏数）；写盘前 YAML safe_load 验证（第一次运行 YAML 引号错误在写盘后被检出→git checkout 回滚→修复后重跑，最终态为验证后写盘）
