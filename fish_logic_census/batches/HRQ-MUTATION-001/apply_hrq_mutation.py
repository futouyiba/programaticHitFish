...
HEAD_NEW = """mutation_provenance:
  ...
  handed_to_rerun: "truth_rebuild_queue.jsonl 248 行（234 AMB+14 slot_tiering）——终局全库顺序还原重跑输入清单（逐鱼顺序推导必做；C9、brooded[ARO/TIL3] 与 form_hold[CSL/CSN1] 由重跑证据终裁——REV-001-R2 补正）"
...
rep(
    ...
    """    known_instances:
      # —— v8 历史快照层（66 名义：47 成员 v9 转正 9 链族+14 slot_tiering 转 queue[RS1 重跑体承载——REV-001-R1 补正]+5 无文件成员 evidence_insufficient_held——裁决 1-A；SINGLE v1 证伪后本清单不计任何族的正式成员）——
      - {batch: CENSUS-B1, program_id: P-B1-GRB-BAKE, blind_hash: 7386b661fbf0c696}""",
    1,
...
GUARD_NEW = f"""    allowed_parameter_axes:
      - anchor(typed：后代空间存在形式——四值+一边界[裁决 4-A 四形式方案]：nest[构建型巢体：石巢/砾脊合一/pebble_mound/泡沫巢/殖民巢/清巢] | egg_mass[利用型附着：岩缝/洞顶/岩面/树根——底质差异归 suitability] | fry_school[移动稚鱼群] | host_brood[蚌宿主]；brooded[口孵/体内携带]=结构级退化链不入本族——待真形重跑终裁[ARO 银龙口哺/TIL3 罗非退化链先例]；form_hold[四形式证据不足挂起]=成员级挂起态不入四值——待真形重跑终裁[CSL/CSN1，REV-001 F1/R3])   # v9 重写：原 intruder_evaluator_context 6+1 值与 18 候选收敛为四形式（轴膨胀根因=四维度塞一轴——关注项经裁决 4 关闭）
