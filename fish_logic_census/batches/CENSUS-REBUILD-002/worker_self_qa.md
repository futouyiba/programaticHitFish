...
2. RBP 同代号异种处理（推导独立、面级守恒判据不适用于异种）：RB-1 摄食面真形不适用于护卵面项——独立推导（面级守恒判据）。✓
3. 冲突序 0：RB-1 移动提案 7 成员的 RS1 轨复核全部同向（BSB FDR SSL SDG TSK WIT YTF）。✓
4. 批内去重：JSB↔ASB（同属参照（源数据层 FishBase 同、页面/Species 层不同——REV-001 F2）——真形同体推导+双 queue 项独立记账）/AGC↔GRB（同种品系——真形复用）。✓
